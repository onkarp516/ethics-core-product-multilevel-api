...
//            String state_code = jsonRequest.get("state_code");
    ...
    //    GSTR1 Credit note Registered Api
    ...
    //    Api for GSTR1 Credit Note Registered outward data excel export
    public InputStream exportExcelGSTR1CRNoteRegData(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"Sr.No", "Date", "Invoice NO", "Particulars","GSTIN/UIN", "Voucher Type","Taxable Amt","Integrated Tax Amt", "Centeral Tax Amt", "State Tax Amt", "Cess Amt", "Tax Amt", "Invoice Amt"};
                    Sheet sheet = workbook.createSheet("exportExcel_GSTR1_CRNote_Reg");
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        Row row = sheet.createRow(rowIdx++);
                        row.createCell(0).setCellValue(i + 1);
                        row.createCell(1).setCellValue(batchNo.get("transaction_date").getAsString());
                        row.createCell(2).setCellValue(batchNo.get("invoice_no").getAsString());
                        row.createCell(3).setCellValue(batchNo.get("ledger_name").getAsString());
                        row.createCell(4).setCellValue(batchNo.get("gst_number").getAsString());
                        ...
                        row.createCell(7).setCellValue(batchNo.get("total_igst").getAsDouble());
                        row.createCell(8).setCellValue(batchNo.get("total_cgst").getAsDouble());
                        row.createCell(9).setCellValue(batchNo.get("total_sgst").getAsDouble());
                        row.createCell(11).setCellValue(batchNo.get("total_tax").getAsDouble());
                        row.createCell(12).setCellValue(batchNo.get("total_amt").getAsDouble());
    ...
    //    GSTR1 Credit note Un-Registered Api
    ...
    //    Api for GSTR1 Credit Note UnRegistered outward data excel export
    public InputStream exportExcelGSTR1CRNoteUnregData(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"Sr.No", "Date", "Invoice NO", "Particulars", "Voucher Type","Taxable Amt","Integrated Tax Amt", "Centeral Tax Amt", "State Tax Amt", "Cess Amt", "Tax Amt", "Invoice Amt"};
                    Sheet sheet = workbook.createSheet("exportExcel_GSTR1_CRNote_UnReg");
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        row.createCell(0).setCellValue(i + 1);
                        row.createCell(1).setCellValue(batchNo.get("transaction_date").getAsString());
                        row.createCell(2).setCellValue(batchNo.get("sales_return_no").getAsString());
                        row.createCell(3).setCellValue(batchNo.get("sundry_debtor_name").getAsString());
//                        row.createCell(4).setCellValue(batchNo.get("gst_number").getAsString());
                        row.createCell(4).setCellValue(batchNo.get("sales_account_name").getAsString());
                        row.createCell(5).setCellValue(batchNo.get("taxable_amt").getAsDouble());
                        row.createCell(6).setCellValue(batchNo.get("total_igst").getAsDouble());
                        row.createCell(7).setCellValue(batchNo.get("total_cgst").getAsDouble());
                        row.createCell(8).setCellValue(batchNo.get("total_sgst").getAsDouble());
                        row.createCell(10).setCellValue(batchNo.get("tax_amt").getAsDouble());
                        row.createCell(11).setCellValue(batchNo.get("total_amount").getAsDouble());
...
//        q.setParameter(3, endDatep);
    ...
    //    Api for GSTR1 HSN screen1 outward data excel export
    public InputStream excelExportGSTR1HsnData(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"HSN/Sac", "Description", "Type of supply", "Total quantity", "Total volume"
                            ,"Tax rate", "Taxable amt", "Integrated Tax Amt", "Centeral Tax Amt", "State Tax Amt","Tax Amt"};
                    Sheet sheet = workbook.createSheet("GSTR1_HSN_ExcelSheet");
                    ...
                    int sumOfVoucherCount = 0;
                    Long totalQty = 0L, total_tax_rate= 0L;
                    Double sumOfTaxableAmt = 0.0, sumOfIGSTAmt = 0.0, sumOfCGSTAmt = 0.0, sumOfSGSTAmt = 0.0, sumOfTaxAmt = 0.0, sumOfInvoiceAmt = 0.0;
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        Row row = sheet.createRow(rowIdx++);
//                        row.createCell(0).setCellValue(i + 1);
                        row.createCell(0).setCellValue(batchNo.get("hsn_no").getAsString());
                        row.createCell(1).setCellValue(batchNo.get("hsn_description").getAsString());
                        row.createCell(2).setCellValue(batchNo.get("supply_type").getAsString());
                        row.createCell(3).setCellValue(batchNo.get("Tqty").getAsLong());
                        row.createCell(4).setCellValue(batchNo.get("total_amount").getAsDouble());
                        row.createCell(5).setCellValue(batchNo.get("tax_rate").getAsDouble());
                        row.createCell(6).setCellValue(batchNo.get("taxable_amt").getAsDouble());
                        row.createCell(7).setCellValue(batchNo.get("igst_amt").getAsDouble());
                        row.createCell(8).setCellValue(batchNo.get("cgst_amt").getAsDouble());
                        row.createCell(9).setCellValue(batchNo.get("sgst_amt").getAsDouble());
                        row.createCell(10).setCellValue(batchNo.get("tax_amt").getAsDouble());

                        totalQty += batchNo.get("Tqty").getAsLong();
                        total_tax_rate += batchNo.get("tax_rate").getAsLong();
                        sumOfTaxableAmt += batchNo.get("taxable_amt").getAsDouble();
                        sumOfCGSTAmt += batchNo.get("cgst_amt").getAsDouble();
                        sumOfSGSTAmt += batchNo.get("sgst_amt").getAsDouble();
                        sumOfIGSTAmt += batchNo.get("igst_amt").getAsDouble();
                    ...
                    Row prow = sheet.createRow(rowIdx++);
                    prow.createCell(0).setCellValue("Total");
                    prow.createCell(3).setCellValue(totalQty);
                    prow.createCell(4).setCellValue(sumOfInvoiceAmt);
                    prow.createCell(5).setCellValue(total_tax_rate);
                    prow.createCell(6).setCellValue(sumOfTaxableAmt);
                    prow.createCell(7).setCellValue(sumOfIGSTAmt);
                    prow.createCell(8).setCellValue(sumOfCGSTAmt);
                    prow.createCell(9).setCellValue(sumOfSGSTAmt);
            ...
            productLogger.error("Failed to load HSN data in excel " + e);
            e.printStackTrace();
            System.out.println("Exception " + e.getMessage());
            throw new RuntimeException("fail to import data to Excel file: " + e.getMessage());
        }
    ...
    //    Api for GSTR1 HSN screen2 outward data excel export
    public InputStream excelExportGSTR1Hsn2Data(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"Sr.No", "Date", "Invoice NO", "Particulars","GSTIN / UIN", "Voucher Type", "UQC","Taxable Amt",
                            "Integrated Tax Amt", "Centeral Tax Amt", "State Tax Amt","Tax Amt"};
                    Sheet sheet = workbook.createSheet("GSTR1_HSN2_ExcelSheet");
                    ...
                    int sumOfVoucherCount = 0;
                    Double sumOfTaxableAmt = 0.0, sumOfIGSTAmt = 0.0, sumOfCGSTAmt = 0.0, sumOfSGSTAmt = 0.0, sumOfTaxAmt = 0.0;
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        row.createCell(2).setCellValue(batchNo.get("invoice_no").getAsString());
                        row.createCell(3).setCellValue(batchNo.get("ledger_name").getAsString());
                        row.createCell(4).setCellValue(batchNo.get("gst_no").getAsString());
                        row.createCell(5).setCellValue(batchNo.get("voucher_type").getAsString());
                        row.createCell(6).setCellValue(batchNo.get("unit_name").getAsString());
                        row.createCell(7).setCellValue(batchNo.get("taxable_amt").getAsDouble());
                        row.createCell(8).setCellValue(batchNo.get("total_igst").getAsDouble());
                        row.createCell(9).setCellValue(batchNo.get("total_cgst").getAsDouble());
                        row.createCell(10).setCellValue(batchNo.get("total_sgst").getAsDouble());
                        row.createCell(11).setCellValue(batchNo.get("total_tax").getAsDouble());
...
//                        sumOfInvoiceAmt += batchNo.get("total_amount").getAsDouble();
                    ...
                    Row prow = sheet.createRow(rowIdx++);
                    prow.createCell(0).setCellValue("Total");
                    prow.createCell(7).setCellValue(sumOfTaxableAmt);
                    prow.createCell(8).setCellValue(sumOfIGSTAmt);
                    prow.createCell(9).setCellValue(sumOfCGSTAmt);
                    prow.createCell(10).setCellValue(sumOfSGSTAmt);
                    prow.createCell(11).setCellValue(sumOfTaxAmt);
    ...
    public JsonObject getGSTR1MainScreen1(HttpServletRequest request) {
        ...
        finalObject.addProperty("total_invoices", voucher_count);
        finalObject.addProperty("start_date", String.valueOf(currentStartDate));
        finalObject.addProperty("end_date", String.valueOf(currentEndDate));
        finalObject.addProperty("responseStatus", HttpStatus.OK.value());
...
//        String searchText = request.getParameter("searchText");
        ...
        String query ="SELECT SUM(tsi.total_base_amount), SUM(tsi.totaligst), SUM(tsi.totalsgst), SUM(tsi.totalcgst), " +
                "SUM(tsi.total_tax), SUM(tsi.total_amount) FROM tranx_sales_invoice_tbl AS tsi LEFT JOIN ledger_master_tbl" +
        ...
        try {
            for (int j = 0; j < B2CscreenData.size(); j++) {
                ...
                if (B2CscreenData != null) {
                    ...
                    inspObj.addProperty("igst_amt", parseDouble(B2CscreenData.get(j)[1].toString()));
                    inspObj.addProperty("sgst_amt", parseDouble(B2CscreenData.get(j)[2].toString()));
                    inspObj.addProperty("cgst_amt", parseDouble(B2CscreenData.get(j)[3].toString()));
                    inspObj.addProperty("tax_amt", parseDouble(B2CscreenData.get(j)[4].toString()));
...
//        String searchText = request.getParameter("searchText");
        ...
        try {
            for (int j = 0; j < B2CscreenData.size(); j++) {
                ...
                System.out.println("inspObj " + B2CscreenData);
                if (B2CscreenData != null) {
                    inspObj.addProperty("voucher_count", parseDouble( B2CscreenData.get(j)[0].toString()));
                    inspObj.addProperty("taxable_amt", parseDouble( B2CscreenData.get(j)[0].toString()));
                    inspObj.addProperty("igst_amt", parseDouble(B2CscreenData.get(j)[1].toString()));
                    inspObj.addProperty("sgst_amt", parseDouble(B2CscreenData.get(j)[2].toString()));
                    inspObj.addProperty("cgst_amt", parseDouble(B2CscreenData.get(j)[3].toString()));
            ...
            for (int j = 0; j < HSNData.size(); j++) {
                ...
                if (HSNData != null) {
                    ...
                    if(HSNData.get(j).length >0) {
                        ...
                        T_invoice_amt = T_invoice_amt + numFormat.numFormat(parseDouble( HSNData.get(j)[10].toString()));
                    }
//                    inspObj.addProperty("hsn_no", HSNData.get(j)[0].toString());
//                    inspObj.addProperty("hsn_description", HSNData.get(j)[1].toString());
//                    inspObj.addProperty("supply_type", HSNData.get(j)[2].toString());
