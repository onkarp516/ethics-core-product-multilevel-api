...
//    public Object createTranxSalesCompInvoices(HttpServletRequest request) throws Exception {
//        ResponseMessage responseMessage = new ResponseMessage();
//        JsonObject object = new JsonObject();
//        TranxSalesCompInvoice mSalesTranx = null;
//        TranxSalesCompInvoice salesInvoice = null;
...
//            insertIntoCompTranxDetailRO(mSalesTranx, tranxType); // for Round Off : cr or dr
//            insertCompDB(mSalesTranx, "AC", tranxType, "CR", "Insert"); // for Additional Charges : cr
//            if (users.getOutlet().getGstApplicable())
//                insertCompDB(mSalesTranx, "DT", tranxType, "CR", "Insert"); // for Duties and Taxes : cr
//            /*** Insert into Day Book : Reporting ***/
...
//    }
    ...
    public Object createTranxSalesInvoices(HttpServletRequest request) throws Exception {
        ...
        if (mSalesTranx != null) {
            ...
            String salesOrderId = "";
            if (paramMap.containsKey("reference_so_id")) salesOrderId = request.getParameter("reference_so_id");
            String paymentMode = request.getParameter("paymentMode");//cash ,credit or online
            Double paidAmt = 0.0;
            if (paramMap.containsKey("paidAmount")) paidAmt = Double.parseDouble(request.getParameter("paidAmount"));
            // createReceiptInvoice(mSalesTranx, users, salesOrderId, paidAmt);
            JsonArray paymentType = new JsonArray();
            ...
            if (paymentMode.equalsIgnoreCase("multi")) {
                ...
                cashAmt = mSalesTranx.getTotalAmount();
    ...
    private void createReceiptInvoice(TranxSalesInvoice newInvoice, Users users, JsonArray paymentType,
                                      Double totalPaidAmt, Double returnAmt, String paymentMode, String key, Double cashAmt) {
    ...
    private void updateReceiptInvoice(TranxSalesInvoice newInvoice, Users users, JsonArray paymentType,
                                      Double totalPaidAmt, Double returnAmt, String paymentMode, String key, Double cashAmt) {
        TransactionTypeMaster tranxType = tranxRepository.findByTransactionCodeIgnoreCase("RCPT");
...
//            insertIntoReceiptPerticualrs(newTranxReceiptMaster, ledgerMaster, "others", cashAmt, key, "cash", tranxType, newInvoice);
    ...
    private TranxSalesInvoice saveIntoInvoice(Users users, HttpServletRequest request, TransactionTypeMaster tranxType, String salesType) throws Exception {
        ...
        invoiceTranx.setBillDate(DateConvertUtil.convertStringToDate(date.toString()));
       ...
       /* if (salesType.equalsIgnoreCase("sales")) {
        ...
        try {
            TransactionTypeMaster tranxTypeMaster = tranxRepository.findByTransactionCodeIgnoreCase("SLS");
...
//    private TranxSalesCompInvoice saveIntoCompInvoice(Users users, HttpServletRequest request, TransactionTypeMaster tranxType, String salesType) throws Exception {
//        Map<String, String[]> paramMap = request.getParameterMap();
//        TranxSalesCompInvoice mSalesTranx = null;
//        Outlet outlet = users.getOutlet();
//        TranxSalesCompInvoice invoiceTranx = new TranxSalesCompInvoice();
...
//                    Branch branch=branchRepository.findByIdAndStatus(mSalesTranx.getBranchId(),true);
    ...
    public void saveIntoSalesInvoiceDetails(JsonArray invoiceDetails, TranxSalesInvoice mSalesTranx, Branch branch, Outlet outlet, Long userId, TransactionTypeMaster tranxType, String salesType, String referenceObj) {
        ...
        for (int i = 0; i < invoiceDetails.size(); i++) {
                    ...
                    if (jsonArray != null && jsonArray.size() > 0) {
                        ...
                        for (JsonElement jsonElement : jsonArray) {
                            ...
                            if (mProduct.getIsInventory() && !refType.equalsIgnoreCase("SLSCHN")) {
                                inventoryCommonPostings.callToInventoryPostings("DR", mSalesTranx.getBillDate(),
                                        mSalesTranx.getId(),
                                        tranxQty + free_qty, branch, outlet, mProduct, tranxType, levelA, levelB,
                                        levelC, units, productBatchNo, batchNo, mSalesTranx.getFiscalYear(), serialNo);
                            }
                        }
                    }
                    flag = true;
            ...
            try {
               /* ProductUnitPacking productUnitPacking = productUnitRepository.findRate(mProduct.getId(),
                ...
                if (mProduct.getIsInventory() && flag && !refType.equalsIgnoreCase("SLSCHN")) {
                    ...
                    inventoryCommonPostings.callToInventoryPostings("DR", mSalesTranx.getBillDate(),
                            mSalesTranx.getId(), tranxQty + free_qty, branch, outlet,
                            mProduct, tranxType, levelA, levelB, levelC, units, productBatchNo, batchNo,
                            mSalesTranx.getFiscalYear(), null);
                    ...
                    closingUtility.stockPosting(outlet, branch, mSalesTranx.getFiscalYear().getId(), batchId,
                            ...
                            mSalesTranx.getTranxCode(), userId, "OUT");
            ...
            else if (refType.equalsIgnoreCase("SLSCHN")) {
...
*/
    ...
    private void insertCompFromDutiesTaxes(TranxSalesCompInvoiceDutiesTaxes mDuties, TranxSalesCompInvoice mSalesTranx, TransactionTypeMaster tranxType, String type, String operation) {
        ...
        ledgerCommonPostings.callToPostingsTranxCode(mDuties.getAmount(), dutiesTaxes, tranxType, dutiesTaxes.getAssociateGroups(), fiscalYear, branch, outlet, mSalesTranx.getBillDate(), mSalesTranx.getId(), mSalesTranx.getSalesInvoiceNo(), type, true, "Consumer Sales", operation, mSalesTranx.getTranxCode());
    }
...
//    }
    ...
    private SalesCounterDTO convertToDTDTO(TranxCounterSales tranxCounterSales) {
        ...
        salesCounterDTO.setTransaction_date(DateConvertUtil.convertDateToLocalDate(tranxCounterSales.getTransactionDate()).toString());
...
//        Integer to = Integer.parseInt(request.getParameter("to"));
    ...
    //    Api for GSTR1 B2B sales outward data
...
//        String query = "SELECT id FROM `tranx_sales_invoice_tbl";
    ...
    private SalesInvoiceDTO convertToCompDTDTO(TranxSalesCompInvoice tranxSaleInvoiceList) {
        ...
        salesInvoiceDTO.setInvoice_date(tranxSaleInvoiceList.getBillDate().toString());
...
//        >>>>>>>>>>>
    ...
    public JsonObject getCounterSalesWithId(HttpServletRequest request) {
        ...
        if (csData != null) {
            ...
            counter_sales_data.addProperty("transaction_dt", DateConvertUtil.convertDateToLocalDate(csData.getTransactionDate()).toString());
    ...
    private ResponseMessage saveIntoSalesCompInvoiceEdit(HttpServletRequest request, String key) {
        ...
        if (salesAccount.getId() != null && invoiceTranx.getId() != null && tranxType.getId() != null) {
            ...
            if (isContains) {
                ...
                ledgerCommonPostings.callToPostings(Double.parseDouble(request.getParameter("taxable_amount")), salesAccount, tranxType, salesAccount.getAssociateGroups(),
                        fiscalYear, branch, outlet, dt, invoiceTranx.getId(), invoiceTranx.getSalesInvoiceNo(), "CR",
                        true, tranxType.getTransactionCode(), "Insert");
            }
        ...
        if (isContains) {
            ...
            if (rf >= 0) {
                ...
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(rf, roundoff, tranxType, roundoff.getAssociateGroups(), fiscalYear,
                        branch, outlet, dt, invoiceTranx.getId(), invoiceTranx.getSalesInvoiceNo(), "CR", true,
...
//        invoiceTranx.setTcs(Double.parseDouble(request.getParameter("tcs")));
    ...
    private ResponseMessage saveIntoSalesInvoiceEdit(HttpServletRequest request, String key) {
        ...
        if (paramMap.containsKey("gstType")) gstType = Integer.parseInt(request.getParameter("gstType"));
        Boolean taxFlag;
        LocalDate date = DateConvertUtil.convertStringToLocalDate(request.getParameter("bill_dt"));
        Date dt = DateConvertUtil.convertStringToDate(request.getParameter("bill_dt"));
        String billDate = request.getParameter("bill_dt");
        ...
        if (salesAccount.getId() != null && invoiceTranx.getId() != null && tranxType.getId() != null) {
            ...
            if (isContains) {
                /* edit ledger tranx if same ledger is modified */
                /**** New Postings Logic *****/
                LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository.findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(salesAccount.getId(),
                        tranxType.getId(), invoiceTranx.getId());
                if (mLedger != null) {
                    if (gstType == 3) mLedger.setAmount(Double.parseDouble(request.getParameter("total_base_amt")));
                    else if (gstType == 1)
                        mLedger.setAmount(Double.parseDouble(request.getParameter("taxable_amount")));
                    mLedger.setTransactionDate(DateConvertUtil.convertStringToDate(billDate));
                    mLedger.setOperations("updated");
                    ledgerTransactionPostingsRepository.save(mLedger);
                }
            } else {
                ...
                /**** New Postings Logic *****/
                if (gstType == 3)
                    ledgerCommonPostings.callToPostings(Double.parseDouble(request.getParameter("total_base_amt")), salesAccount, tranxType,
                            salesAccount.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(), invoiceTranx.getOutlet(), dt, invoiceTranx.getId(),
                            invoiceTranx.getSalesInvoiceNo(), "CR", true, tranxType.getTransactionCode(), "Insert");
                else if (gstType == 1)
                    ledgerCommonPostings.callToPostings(Double.parseDouble(request.getParameter("taxable_amount")), salesAccount, tranxType,
                            salesAccount.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(), invoiceTranx.getOutlet(), dt, invoiceTranx.getId(),
                            invoiceTranx.getSalesInvoiceNo(), "CR", true, tranxType.getTransactionCode(), "Insert");
            }
        }/* end of calling store procedure for Sales Account updating opening and closing */
        ...
        if (sundryDebtors.getId() != null && invoiceTranx.getId() != null && tranxType.getId() != null) {
            ...
            if (isContains) {
                ...
                /* insert ledger tranx if ledger is changed */
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(Double.parseDouble(request.getParameter("totalamt")), sundryDebtors, tranxType, sundryDebtors.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(),
                        invoiceTranx.getOutlet(), dt, invoiceTranx.getId(), invoiceTranx.getSalesInvoiceNo(), "DR", true, tranxType.getTransactionCode(), "Insert");
            }
        }  /* end of calling store procedure for updating opening and closing of Sundry Creditors  */
        ...
        if (isContains) {
            ...
            if (rf >= 0) {
                ...
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(rf, roundoff, tranxType, roundoff.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(), invoiceTranx.getOutlet(),
                        dt, invoiceTranx.getId(), invoiceTranx.getSalesInvoiceNo(), "DR", true, tranxType.getTransactionCode(), "Insert");
            } else {
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(rf, roundoff, tranxType, roundoff.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(), invoiceTranx.getOutlet(),
                        dt, invoiceTranx.getId(), invoiceTranx.getSalesInvoiceNo(), "CR", true, tranxType.getTransactionCode(), "Insert");
            }
        }
        /* end of inserting into Sundry Creditors JSON Object */
        invoiceTranx.setTotalAmount(Double.parseDouble(request.getParameter("totalamt")));
            ...
            if (additionalChgLedger3 != null) {
                ...
                if (isContains) {
                    ...
                    if (mLedger != null) {
                        ...
                        /**** New Postings Logic *****/
                        ledgerCommonPostings.callToPostings(mSalesTranx.getAdditionLedgerAmt3(), mSalesTranx.getAdditionLedger3(), tranxType,
                                mSalesTranx.getAdditionLedger3().getAssociateGroups(), mSalesTranx.getFiscalYear(), mSalesTranx.getBranch(), mSalesTranx.getOutlet(),
                                mSalesTranx.getBillDate(), mSalesTranx.getId(), mSalesTranx.getSalesInvoiceNo(), mSalesTranx.getAdditionLedgerAmt3() > 0 ? "CR" : "DR",
                                true, tranxType.getTransactionCode(), "Insert");
                    }
                }
            }
        ...
        try {
            ...
            if (mSalesTranx != null) {
                ...
                for (Long mDblist : dbList) {
                    if (!mInputList.contains(mDblist)) {
                        /**** New Postings Logic *****/
                        LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository.findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(mDblist,
                                tranxType.getId(), invoiceTranx.getId());
                        if (mLedger != null) {
                            mLedger.setAmount(0.0);
                            mLedger.setTransactionDate(invoiceTranx.getBillDate());
        ...
        for (JsonElement mAddCharges : additionalCharges) {
            ...
            if (object.has("amt") && !object.get("amt").getAsString().equalsIgnoreCase("")) {
                ...
                if (isContains) {
                    ...
                    /* insert */
                    /**** New Postings Logic *****/
                    ledgerCommonPostings.callToPostings(amount, addcharges, tranxTypeMater, addcharges.getAssociateGroups(), mSalesInvoie.getFiscalYear(),
                            mSalesInvoie.getBranch(), mSalesInvoie.getOutlet(), mSalesInvoie.getBillDate(), mSalesInvoie.getId(), mSalesInvoie.getSalesInvoiceNo(),
                            "CR", true, "Sales Invoice", "Insert");
                }
            }
        }
                    ...
                    if (details_id != 0) {
                        ...
                        if(productRow!=null) {
                            if (mSalesTranx.getBillDate().compareTo(productRow.getTranxDate()) == 0 &&
                                ...
                                Double closingStk = closingUtility.CAL_CR_STOCK(productRow.getOpeningStock(), invoiceUnits.getQty(), free_qty);
                                productRow.setQty(invoiceUnits.getQty() + free_qty);
    ...
    public JsonObject getInvoiceBillPrint(HttpServletRequest request) {
        ...
        try {
            ...
            if (salesInvoice != null) {
                ...
                debtorsObject.addProperty("supplier_phone", salesInvoice.getSundryDebtors().getMobile() != null ? salesInvoice.getSundryDebtors().getMobile().toString() : "");

                SimpleDateFormat df = new SimpleDateFormat("HH:mm");
                Date today_dt = new Date();
                String today = df.format(today_dt);
                debtorsObject.addProperty("time", today);
                ...
                /* getting Units of Sales Invoice*/
                List<TranxSalesInvoiceDetailsUnits> unitDetails = tranxSalesInvoiceDetailsUnitRepository.findBySalesInvoiceIdAndStatus(salesInvoice.getId(), true);
                List<TranxSalesInvoiceDutiesTaxes> taxDetails = salesDutiesTaxesRepository.findTaxes(salesInvoice.getId(), true);
                for (TranxSalesInvoiceDutiesTaxes mTax : taxDetails) {
                    JsonObject taxObject = new JsonObject();
                    String ledgerName = mTax.getDutiesTaxes().getLedgerName();
                    if (ledgerName.equalsIgnoreCase("OUTPUT CGST 2.5") || ledgerName.equalsIgnoreCase("OUTPUT SGST 2.5")) {
                        gst5 = gst5 + mTax.getAmount();
                    } else if (ledgerName.equalsIgnoreCase("OUTPUT SGST 6") || ledgerName.equalsIgnoreCase("OUTPUT CGST 6")) {
//                        taxObject.addProperty("taxGroupSgst", "12");
                        gst12 = gst12 + mTax.getAmount();
                    } else if (ledgerName.equalsIgnoreCase("OUTPUT CGST 1.5") || ledgerName.equalsIgnoreCase("OUTPUT SGST 1.5")) {
//                        taxObject.addProperty("taxGroupCgst", "12");
                        gst3 = gst3 + mTax.getAmount();
                    } else if (ledgerName.equalsIgnoreCase("OUTPUT CGST 9") || ledgerName.equalsIgnoreCase("OUTPUT SGST 9")) {
                        gst18 = gst18 + mTax.getAmount();
                    } else if (ledgerName.equalsIgnoreCase("OUTPUT CGST 14") || ledgerName.equalsIgnoreCase("OUTPUT SGST 14")) {
                        gst28 = gst28 + mTax.getAmount();
                    } else {
                        gst0 = gst0 + mTax.getAmount();
                    }
                }
                JsonObject taxObj = new JsonObject();
                taxObj.addProperty("gst_0", gst0);
                taxObj.addProperty("gst_3", gst3);
                taxObj.addProperty("gst_5", gst5);
                taxObj.addProperty("gst_12", gst12);
                taxObj.addProperty("gst_18", gst18);
                taxObj.addProperty("gst_28", gst28);
                taxArray.add(taxObj);
                JsonArray productDetails = new JsonArray();
                unitDetails.forEach(mUnit -> {
                    JsonObject mObject = new JsonObject();
                ...
                finalResult.add("tax_data", taxArray);
                ...
                invoiceObject.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(salesChallan.getBillDate()).toString());
...
//                debtorsObject.addProperty("supplier_phone", counterSales.getSundryDebtors().getMobile());
            ...
            salesInvoiceLogger.error("Error in getInvoiceBillPrint :->" + e1.getMessage());
            System.out.println(e1.getMessage());
            e1.printStackTrace();
            finalResult.addProperty("message", "error");
            finalResult.addProperty("responseStatus", HttpStatus.FORBIDDEN.value());
...
//        invoiceTranx.setOutlet(outlet);
//        LocalDate date = LocalDate.parse(request.getParameter("bill_dt"));
//        invoiceTranx.setTransactionDate(date);
//        /* fiscal year mapping*/
//        FiscalYear fiscalYear = generateFiscalYear.getFiscalYear(date);
//        if (fiscalYear != null) {
//            invoiceTranx.setFiscalYear(fiscalYear);
...
//    }
    ...
    public ResponseMessage createCounterSales(HttpServletRequest request) {
        ...
        invoiceTranx.setOutlet(outlet);
        LocalDate date = LocalDate.parse(request.getParameter("bill_dt"));
        invoiceTranx.setTransactionDate(DateConvertUtil.convertStringToDate(request.getParameter("bill_dt")));
        ...
        for (int i = 0; i < invoiceDetails.size(); i++) {
            ...
            try {
                if (object.get("is_batch").getAsBoolean()) {
                    ...
                    invoiceUnits.setProductBatchNo(productBatchNo);
//                    if (i == 0) {
//                        mSalesTranx.setCounterSaleSrNo(object.get("counterNo").getAsLong());
//                        if (object.has("mobile_number") && !object.get("mobile_number").getAsString().equalsIgnoreCase(""))
//                            mSalesTranx.setMobileNumber(Long.parseLong(object.get("mobile_number").getAsString()));
//                        if (object.has("paymentMode"))
//                            mSalesTranx.setPaymentMode(object.get("paymentMode").getAsString());
//                        counterSaleRepository.save(mSalesTranx);
//                    }
                }
...
//    }
    ...
    public ResponseMessage updateCounterSales(HttpServletRequest request) {
     ...
     //   invoiceTranx.setTransactionDate(DateConvertUtil.convertStringToDate(request.getParameter("bill_dt")));
        ...
        LocalDate newInvoiceDate = DateConvertUtil.convertStringToLocalDate(request.getParameter("bill_dt"));
        Date strDt = DateConvertUtil.convertStringToDate(request.getParameter("bill_dt"));
        if (newInvoiceDate.isEqual(DateConvertUtil.convertDateToLocalDate(invoiceTranx.getTransactionDate()))) {
            strDt = invoiceTranx.getTransactionDate();
        }
        ...
        /* End of fiscal year mapping*/
        //   invoiceTranx.setCounterSaleSrNo(Long.parseLong(request.getParameter("sales_sr_no")));
        invoiceTranx.setCounterSaleNo(request.getParameter("bill_no"));
        invoiceTranx.setNarrations(request.getParameter("narration"));
        // invoiceTranx.setCustomerName(request.getParameter("customer_name"));
        // invoiceTranx.setRoundoff(Double.parseDouble(request.getParameter("roundoff")));
        //invoiceTranx.setMobileNumber(Long.parseLong(request.getParameter("mobile_number")));
        //     invoiceTranx.setTotalBaseAmt(Double.parseDouble(request.getParameter("total_base_amt")));
...
//        invoiceTranx.setFreeQty(Double.valueOf(request.getParameter("total_free_qty")));
    ...
    public JsonArray getCounterSalesData(TranxCounterSales salesInvoice) {
        ...
        finalResult.addProperty("billDate", DateConvertUtil.convertDateToLocalDate(salesInvoice.getTransactionDate()).toString());
    ...
    public JsonObject getSalesVerificationById(HttpServletRequest request) {
        ...
        opqty = productOpeningStocksRepository.findOpeningStockWithoutBranchFilter(users.getOutlet().getId(), id, levelAId, levelBId, levelCId, unitId, batchId, fiscalYear.getId());
       /* if(crqty<qty){//5<2
            opqty = productOpeningStocksRepository.findOpeningStockWithoutBranchFilter(users.getOutlet().getId(), id, levelAId, levelBId, levelCId, unitId, batchId, fiscalYear.getId());
        }*/
...
//        Map<String, String[]> paramMap = request.getParameterMap();
        ...
        for (TranxSalesInvoice invoices : saleInvoice) {
            ...
            response.addProperty("sundry_debtor_id", invoices.getSundryDebtors().getId());
            response.addProperty("sale_account_name", invoices.getSalesAccountLedger().getLedgerName());
            response.addProperty("transaction_date", DateConvertUtil.convertDateToLocalDate(invoices.getBillDate()).toString());
    ...
    public JsonObject getCounterSalesDetailsData(HttpServletRequest request) {
        ...
        if (paramMap.containsKey("payment_mode")) {
            ...
            if (paymentMode.equalsIgnoreCase("all")) {
                if (branch != null)
                    counterSales = counterSaleRepository.findByOutletIdAndBranchIdAndStatus(
                            users.getOutlet().getId(), branch.getId(), true);
                else {
        ...
        for (TranxCounterSales mSales : counterSales) {
            ...
            int i = 0;
            Double totalAmt = 0.0;
            List<TranxCounterSalesDetailsUnits> detailsUnits = tranxCSDetailsUnitsRepository.
                    findByCounterSalesIdAndStatusAndTransactionStatus(mSales.getId(), true, 1L);
...
//            String paymentMode = request.getParameter("payment_mode");
//            if (branch != null)
//                counterSales = counterSaleRepository.findByOutletIdAndBranchIdAndStatusAndTransactionDateAndTransactionStatusAndPaymentModeIgnoreCase(users.getOutlet().getId(), branch.getId(), true, date, 1L, paymentMode);
//            else {
//                counterSales = counterSaleRepository.findByOutletIdAndStatusAndTransactionDateAndAndTransactionStatusAndPaymentModeIgnoreCaseAndBranchIsNull(users.getOutlet().getId(), true, date, 1L, paymentMode);
//            }
//        } else {
//            if (branch != null)
//                counterSales = counterSaleRepository.findByOutletIdAndBranchIdAndStatusAndTransactionDateAndTransactionStatus(users.getOutlet().getId(), branch.getId(), true, date, 1L);
//            else
//                counterSales = counterSaleRepository.findByOutletIdAndStatusAndTransactionDateAndAndTransactionStatusAndBranchIsNull(users.getOutlet().getId(), true, date, 1L);
//        }
//        JsonArray row = new JsonArray();
//
//        for (TranxCounterSales mSales : counterSales) {
...
//            int i = 0;
//            Double totalAmt = 0.0;
//            List<TranxCounterSalesDetailsUnits> detailsUnits = tranxCSDetailsUnitsRepository.findByCounterSalesIdAndStatus(mSales.getId(), true);
//            for (TranxCounterSalesDetailsUnits mUnits : detailsUnits) {
//                JsonObject object = new JsonObject();
//                object.addProperty("id", mSales.getId());
//                object.addProperty("details_id", mUnits.getId());
//                object.addProperty("counterId", mSales.getId());
//                object.addProperty("counterNo", mSales.getCounterSaleSrNo());
//                object.addProperty("payment_mode", mSales.getPaymentMode());
//                object.addProperty("mobile_number", mSales.getMobileNumber());
//                object.addProperty("productId", mUnits.getProduct().getId());
//                object.addProperty("productName", mUnits.getProduct().getProductName());
...
//    }
    ...
    public Object GSTR1B2CLSaleInvoiceDetails(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        String startDate = request.get("startDate");
        String endDate = request.get("endDate");
        Long state_code = Long.parseLong(request.get("state_code"));
        Long tax_rate = Long.parseLong(request.get("tax_rate"));
        LocalDate endDatep = null;
        LocalDate startDatep = null;
        Boolean flag = false;
        List saleInvoice = new ArrayList<>();
        ...
        GenericDTData genericDTData = new GenericDTData();
        try {
            String query = "SELECT tsi.id FROM `tranx_sales_invoice_tbl` AS tsi LEFT JOIN ledger_master_tbl" +
                    " ON tsi.sundry_debtors_id = ledger_master_tbl.id LEFT JOIN" +
                    " tranx_sales_invoice_details_units_tbl AS tsidu ON tsi.id = tsidu.sales_invoice_id" +
                    " WHERE tsi.status=1 AND tsi.total_amount > 250000 AND ledger_master_tbl.taxable=0 AND tsidu.igst=" + tax_rate + " AND ledger_master_tbl.state_code=" + state_code;
            ...
            if (!searchText.equalsIgnoreCase("")) {
//
                query = query + " AND (bill_date LIKE '%" + searchText + "%'" + "OR sales_invoice_no LIKE '%"
                        + searchText + "%'"  +"OR ledger_master_tbl.ledger_name LIKE '%" + searchText + "%'" + "OR tsi.total_amount LIKE '%" + searchText + "%'"
                        + "OR tsi.total_base_amount LIKE '%" + searchText + "%'" + "OR tsi.total_tax LIKE '%"
                        + searchText + "%'" + "OR tsi.totaligst LIKE '%" + searchText + "%'" + "OR tsi.totalcgst LIKE '%"
                        + searchText +"%'" + "OR tsi.totalsgst LIKE '%" + searchText +  "%'" + "OR tsi.total_amount LIKE '%" + searchText + "%' )";
            }
            String jsonToStr = request.get("sort");
            JsonObject jsonObject = new Gson().fromJson(jsonToStr, JsonObject.class);
            if (!jsonObject.get("colId").toString().equalsIgnoreCase("null") && jsonObject.get("colId").getAsString() != null) {
    ...
    public Object GSTR1B2CSSaleInvoiceDetails(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        try {
            String query = "SELECT tsi.id FROM `tranx_sales_invoice_tbl` AS tsi LEFT JOIN ledger_master_tbl" +
                    " ON tsi.sundry_debtors_id = ledger_master_tbl.id LEFT JOIN" +
                    " tranx_sales_invoice_details_units_tbl AS tsidu ON tsi.id = tsidu.sales_invoice_id" +
                    " WHERE tsi.status=1 AND tsi.total_amount <= 250000 AND ledger_master_tbl.taxable=0 AND tsidu.igst=" + tax_rate + " AND ledger_master_tbl.state_id=" + state_id;
...
//
                query = query + " AND (bill_date LIKE '%" + searchText + "%'" + "OR sales_invoice_no LIKE '%"
                        ...
                        + "OR tsi.total_base_amount LIKE '%" + searchText + "%'" + "OR tsi.total_tax LIKE '%"
                        + searchText + "%'" + "OR tsi.totaligst LIKE '%" + searchText + "%'" + "OR tsi.totalcgst LIKE '%"
                        + searchText + "%'" + "OR tsi.totalsgst LIKE '%" + searchText + "%' )";
            }
            String jsonToStr = request.get("sort");
            JsonObject jsonObject = new Gson().fromJson(jsonToStr, JsonObject.class);
            if (!jsonObject.get("colId").toString().equalsIgnoreCase("null") && jsonObject.get("colId").getAsString() != null) {
...
//            q.setParameter(3, 3);
    ...
    public Object listOfCounterSales(HttpServletRequest req) {
        ...
        try {
            ...
            String sql = "SELECT distinct(counter_sales_id) FROM `tranx_counter_sales_details_units_tbl` LEFT JOIN " +
                    "tranx_counter_sales_tbl ON tranx_counter_sales_details_units_tbl.counter_sales_id=tranx_counter_sales_tbl.id " +
                    "WHERE tranx_counter_sales_details_units_tbl.status=1 AND tranx_counter_sales_details_units_tbl.transaction_status=1 AND " +
                    "tranx_counter_sales_tbl.outlet_id=" + users.getOutlet().getId();
            if (users.getBranch() != null) {
            ...
            for (Object mSalesList : salesList) {
                ...
                object.addProperty("saleDate",
                        counterSale.getCounterSalesDate() != null ? counterSale.getCounterSalesDate().toString() : "");
    ...
    public JsonObject findCounterSalesById(HttpServletRequest request) {
        ...
        try {
            ...
            if (salesInvoice != null) {
                ...
                finalResult.addProperty("roundoff", salesInvoice.getRoundoff());
                finalResult.addProperty("paymentMode", salesInvoice.getPaymentMode());
                List<TranxCounterSalesDetailsUnits> unitsArray = tranxCSDetailsUnitsRepository.
                        findByCounterSalesIdAndStatusAndTransactionStatus(salesInvoice.getId(), true, 1L);
                for (TranxCounterSalesDetailsUnits mUnits : unitsArray) {
                    JsonObject unitsJsonObjects = new JsonObject();
                    unitsJsonObjects.addProperty("details_id", mUnits.getId());
                    unitsJsonObjects.addProperty("product_id", mUnits.getProduct().getId());
    ...
    public Object customerProductsHistory(Map<String, String> jsonReq, HttpServletRequest req) {
        ...
        try {
            ...
            String sql = "SELECT * FROM `tranx_sales_comp_details_units_tbl` AS tscdut LEFT JOIN " +
                    "tranx_sales_comp_invoice_tbl AS tscit ON" + " sales_invoice_id=tscit.id WHERE " +
                    "tscit.mobile_number=" + mobileNo + " AND tscit.status=1 AND tscit.outlet_id=" + users.getOutlet().getId();
...
//            insertIntoCompTranxDetailRO(salesInvoice, tranxType); // for Round Off : cr or dr
            ...
            if (paramMap.containsKey("reference_so_id")) salesOrderId = request.getParameter("reference_so_id");
            Double paidAmt = 0.0;
            if (paramMap.containsKey("paidAmount"))
                paidAmt = Double.parseDouble(request.getParameter("paidAmount"));
            //createReceiptInvoice(mSalesTranx, users, salesOrderId, paidAmt);
            JsonArray paymentType = new JsonArray();
            if (paymentMode.equalsIgnoreCase("multi")) {
    ...
    public JsonObject getSellingHistoryList(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        Long ledger_id = Long.parseLong(request.getParameter("ledger_id"));

        try {

            List<Object[]> results = entityManager.createNativeQuery(
                            "SELECT " +
                                    "    COUNT(sdu.product_id) AS no_of_times_bought, " +
                                    "    SUM(sdu.qty) AS total_quantity, " +
                                    "    pt.product_name " +
                                    "FROM tranx_sales_invoice_details_units_tbl sdu " +
                                    "JOIN tranx_sales_invoice_tbl si ON sdu.sales_invoice_id = si.id " +
                                    "JOIN ledger_master_tbl lm ON si.sundry_debtors_id = lm.id " +
                                    "LEFT JOIN product_tbl pt ON sdu.product_id = pt.id " +
                                    "WHERE lm.id = :ledgerId " +
                                    "GROUP BY pt.product_name " +
                                    "ORDER BY no_of_times_bought DESC")
                    .setParameter("ledgerId", ledger_id)
                    .getResultList();

            JsonArray resultArray = new JsonArray();
            for (Object[] row : results) {
                JsonObject response = new JsonObject();
                response.addProperty("no_of_times_bought", ((BigInteger) row[0]).longValue());
                response.addProperty("total_quantity", (Double) row[1]);
                response.addProperty("product_name", (String) row[2]);
            ...
            output.addProperty("message", "success");
            output.addProperty("responseStatus", HttpStatus.OK.value());
            output.add("data", resultArray);
            return output;

        } catch (Exception e) {
            // Handle exceptions here
            JsonObject errorOutput = new JsonObject();
            errorOutput.addProperty("message", "Error occurred");
            errorOutput.addProperty("responseStatus", HttpStatus.INTERNAL_SERVER_ERROR.value());
            return errorOutput;
        }
    }

    public JsonObject getNonSellingProductsList(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        Long ledger_id = Long.parseLong(request.getParameter("ledger_id"));
        ...
        try {
            List<Object[]> results = entityManager.createNativeQuery(
                            "SELECT pt.product_name, DATEDIFF(CURDATE(), DATE(MAX(tsi.bill_date))) AS days_difference " +
                                    "FROM product_tbl pt " +
                                    "JOIN tranx_sales_invoice_details_units_tbl tsidu ON pt.id = tsidu.product_id " +
                                    "JOIN tranx_sales_invoice_tbl tsi ON tsidu.sales_invoice_id = tsi.id " +
                                    "WHERE tsi.sundry_debtors_id = :ledgerId AND DATEDIFF(CURDATE(), DATE(tsi.bill_date)) >= :days " +
                                    "GROUP BY pt.product_name")
                    .setParameter("ledgerId", ledger_id)
            ...
            for (Object[] row : results) {
                JsonObject response = new JsonObject();
                response.addProperty("product_name", (String) row[0]);
                response.addProperty("days_difference", ((BigInteger) row[1]).longValue());
                resultArray.add(response);
            }
