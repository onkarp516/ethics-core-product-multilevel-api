                ...
                for (int s = 1; s < sheet.getPhysicalNumberOfRows(); s++) {
                    ...
                    if (row.getCell(1) == null) {
                        ...
                        if (ledgerMaster == null) {
                            ...
                            if (mobile != null && !mobile.isEmpty()) ledgerMaster.setMobile(Long.parseLong(mobile));
        ...
        try {
            ...
            /*if (Boolean.parseBoolean(request.getParameter("taxable"))) {
                ...
                if (request.getParameter("slug").equalsIgnoreCase("sundry_creditors") || request.getParameter("slug").equalsIgnoreCase("sundry_debtors")) {
                    ...
                    if(paramMap.containsKey("productDiscount1")){
                        ledgerMaster.setProductDiscount1(Double.valueOf(request.getParameter("productDiscount1")));
                    }
                    if (paramMap.containsKey("productDiscount2")){
                        ledgerMaster.setProductDiscount2(Double.valueOf(request.getParameter("productDiscount2")));
                    }else ledgerMaster.setProductDiscount2(0.0);
                    if (paramMap.containsKey("productDiscount3")){
                        ledgerMaster.setProductDiscount3(Double.valueOf(request.getParameter("productDiscount3")));
                    }else ledgerMaster.setProductDiscount3(0.0);
                    if (paramMap.containsKey("productDiscount4")){
                        ledgerMaster.setProductDiscount4(Double.valueOf(request.getParameter("productDiscount4")));
                    }else ledgerMaster.setProductDiscount4(0.0);
                    if (paramMap.containsKey("billDiscount")){
                        ledgerMaster.setBillDiscount(Double.valueOf(request.getParameter("billDiscount")));
                    }
...
//            ledgerMaster.setOpeningBal(0.0);
    ...
    private void updateOpeningBalance(LedgerMaster mLedger, HttpServletRequest request, Long usersId) {
        ...
        if (openingDetailsJson.size() > 0) {
            for (JsonElement mList : openingDetailsJson) {
                ...
                if (object.has("invoice_date") && !object.get("invoice_date").getAsString().toLowerCase().contains("invalid"))
                    openingDetails.setInvoice_date(LocalDate.parse(object.get("invoice_date").getAsString()));
                if (object.get("due_days").getAsString().equalsIgnoreCase("")) openingDetails.setDue_days(0L);
                else openingDetails.setDue_days(Long.parseLong(object.get("due_days").getAsString()));
                if (object.get("bill_amt").getAsString().equalsIgnoreCase("")) openingDetails.setBill_amt(0.0);
                else openingDetails.setBill_amt(Double.parseDouble(object.get("bill_amt").getAsString()));
                ...
                else openingDetails.setInvoice_bal_amt(Double.parseDouble(object.get("invoice_bal_amt").getAsString()));
    ...
    private void insertOpeningBalanceWithInvoice(LedgerMaster mLedger, JsonArray openingDetailsJson, Long userId) {
        if (openingDetailsJson.size() > 0) {
            for (JsonElement mList : openingDetailsJson) {
                ...
                if (object.get("due_days").getAsString().equalsIgnoreCase("")) openingBalDetails.setDue_days(0L);
                else openingBalDetails.setDue_days(Long.parseLong(object.get("due_days").getAsString()));
                if (object.get("bill_amt").getAsString().equalsIgnoreCase("")) openingBalDetails.setBill_amt(0.0);
                else openingBalDetails.setBill_amt(Double.parseDouble(object.get("bill_amt").getAsString()));
                if (object.get("invoice_paid_amt").getAsString().equalsIgnoreCase(""))
    ...
    private void updateLicenseDetails(LedgerMaster mLedger, HttpServletRequest request) {
    ...
    private void insertIntoLicenseDetails(LedgerMaster mLedger, HttpServletRequest request) {
        String licenseJson = request.getParameter("licensesDetails");
        JsonParser parser = new JsonParser();
        JsonElement licenseElements = parser.parse(licenseJson);
        JsonArray licenseDetails = licenseElements.getAsJsonArray();
...
//        result = getLedgers("Indirect Expenses", users.getOutlet().getId());
    ...
    public JsonObject getAllLedgers(HttpServletRequest request) {
        ...
        for (LedgerMaster balanceSummary : balanceSummaries) {
            ...
            try {
                Double openingBalance = ledgerRepository.findOpeningBalance(balanceSummary.getId());//1000
...
//            LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository.findByLedgerMasterIdAndStatus(balanceSummary.getId(), true);
    ...
    public Object getAllLedgersPagination(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        Integer pageNo = 1;
        Integer pageSize = 50;
        if (paramMap.containsKey("pageNo")) pageNo = Integer.parseInt(request.get("pageNo"));
        if (paramMap.containsKey("pageSize")) pageSize = Integer.parseInt(request.get("pageSize"));
...
//            String query = "SELECT id FROM `ledger_master_tbl` WHERE status=1 AND outlet_id=" + users.getOutlet().getId();
            String query = " SELECT ledger_master_tbl.id FROM ledger_master_tbl LEFT JOIN principles_tbl ON ledger_master_tbl.principle_id = principles_tbl.id "
                    + " LEFT JOIN principle_groups_tbl ON ledger_master_tbl.principle_groups_id = principle_groups_tbl.id" +
                    " LEFT JOIN foundations_tbl ON ledger_master_tbl.foundation_id = foundations_tbl.id WHERE ledger_master_tbl.status = 1"
                    + " AND ledger_master_tbl.outlet_id=" + users.getOutlet().getId();
            if (users.getBranch() != null) {
...
//                query = query + " AND (ledger_name LIKE '%" + searchText + "%')";
                query = query + " AND (" +
                        ...
                        "%' OR principles_tbl.principle_name LIKE '%" + searchText +
                        "%' OR principle_groups_tbl.group_name LIKE '%" + searchText +
                        "%' OR foundations_tbl.foundation_name LIKE '%" + searchText +
                        "%')";
            }
      ...
      /*      Query q1 = entityManager.createNativeQuery(query1, LedgerMaster.class);
            ...
            String query1 = "SELECT COUNT(ledger_master_tbl.id) as totalcount FROM ledger_master_tbl WHERE " + "ledger_master_tbl.status=? AND ledger_master_tbl.outlet_id=?";
            if (users.getBranch() != null) {
            ...
            GenericDatatable<LedgerMasterDTO> data = new GenericDatatable<>(ledgerMasterDTOList, ledgerMasterList.size(), pageNo, pageSize, total_pages);
...
//                } else {
                ...
                jsonObject.addProperty("productDiscount1",mLedger.getProductDiscount1()!= null ? mLedger.getProductDiscount1():0.0);
                jsonObject.addProperty("billDiscount",mLedger.getBillDiscount()!= null ? mLedger.getBillDiscount(): 0.0);
                /* pune visit changes */
                ...
                jsonObject.addProperty("defaultBank", mLedger.getColumnR());
...
//            lMaster = ledgerRepository.findDuplicateWithCode(users.getOutlet().getId(), branchId, Long.parseLong(request.getParameter("principle_id")), pgroupId, request.getParameter("ledger_code").toLowerCase(), true);
    ...
    public Object ledgerTransactionsList(HttpServletRequest request) {
        ...
        try {
            ...
            if (!searchKey.equalsIgnoreCase("")) {
                query = query + " AND (ledger_code LIKE '%" + searchKey + "%' OR ledger_name LIKE '%" +
                        searchKey + "%' " + "OR city LIKE '%" + searchKey + "%' OR mobile LIKE '%" + searchKey + "%')";
            }
           /* Query q = entityManager.createNativeQuery(query, LedgerMaster.class);
            ledgerMasters = q.getResultList();*/
            query = query + " ORDER BY ledger_name LIMIT 50 ";
            Query q = entityManager.createNativeQuery(query);
            ledgerMasters = q.getResultList();
            if (ledgerMasters != null && ledgerMasters.size() > 0) {
                for (Object mLedger : ledgerMasters) {
                    ...
                    mObject.addProperty("prodDiscount1", mDetails.getProductDiscount1() != null ? mDetails.getProductDiscount1():0.0);
                    mObject.addProperty("billDiscount", mDetails.getBillDiscount() != null ? mDetails.getBillDiscount():0.0);
    ...
    public Object ledgerTransactionsDetails(HttpServletRequest request) {
        ...
        if (ledgerMasters != null) {
            ...
            mObject.addProperty("productDis1",ledgerMasters.getProductDiscount1());
            mObject.addProperty("billDiscount",ledgerMasters.getBillDiscount());
