...
public class ProductBatchService {
    ...
    public JsonObject getProductBatch(HttpServletRequest request) {
     ...
     /*   Long sub_category_id = null;
        ...
        if (paramMap.containsKey("unit_id") && !request.getParameter("unit_id").equalsIgnoreCase(""))
            unit_id = Long.parseLong(request.getParameter("unit_id"));

        LocalDate invoiceDate = null;
        if (paramMap.containsKey("invoice_date") && !request.getParameter("invoice_date").equalsIgnoreCase("")) {
            invoiceDate = LocalDate.parse(request.getParameter("invoice_date"));
        }
...
//        List<ProductBatchNo> productbatch = productBatchNoRepository.findByBatchList(product_id, users.getOutlet().getId(), true, level_a_id, level_b_id, level_c_id, unit_id);
        List<ProductBatchNo> productbatch = productBatchNoRepository.findByBatchListNew(product_id, users.getOutlet().getId(), true, level_a_id, level_b_id, level_c_id);
    ...
    public Object batchTransactionsDetails(HttpServletRequest request) {
        ...
        try {
            ...
            if (productBatchNo != null) {
                ...
                batchObject.addProperty("minMargin", productBatchNo.getMinMargin());
...
//                mproductBatchNo.setBarcode(Double.parseDouble(request.getParameter("barcode")));
    ...
    //API for batch details in product edit screen
        ...
        Long package_id = null;*/
            ...
            if (productbatch != null) {
                for (ProductBatchNo mBatch : productbatch) {
                    ...
                    object.addProperty("min_rate_c", mBatch.getMinRateC() != null ? mBatch.getMinRateC() : 0.0);
                    object.addProperty("net_rate", mBatch.getCosting() != null ? mBatch.getCosting() : 0.0);
                    double salesRateWithTax = mBatch.getSalesRate() != null ? mBatch.getSalesRate() : 0.0;
                    if (mBatch.getProduct().getTaxMaster() != null)
                        salesRateWithTax = (mBatch.getSalesRate() != null ? mBatch.getSalesRate() : 0.0) + (((mBatch.getSalesRate() != null ? mBatch.getSalesRate() : 0.0) * mBatch.getProduct().getTaxMaster().getIgst()) / 100);
                    object.addProperty("sales_rate_with_tax", salesRateWithTax);
                    object.addProperty("manufacturing_date", mBatch.getManufacturingDate() != null ? mBatch.getManufacturingDate().toString() : "");
                    object.addProperty("min_margin", mBatch.getMinMargin() != null ? mBatch.getMinMargin() : 0.0);
                    object.addProperty("b_rate", mBatch.getMrp() != null ? mBatch.getMrp() : 0.0);
