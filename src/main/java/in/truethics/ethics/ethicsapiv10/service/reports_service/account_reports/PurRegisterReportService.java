...
public class PurRegisterReportService {
    ...
    private static final Logger stockLogger = LogManager.getLogger(PurRegisterReportService.class);
...
//                inside.addProperty("particulars", mInvoice.getSundryCreditors().getLedgerName());
...
//            res.add("credit", creditArray);
    ...
    public InputStream exportExcelPurchaseRegReport(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"MONTH", "Debit A/C","Credit A/C","Closing Balance","Type C/D"};
                    if (mfgShow)
                        headers = new String[]{"MONTH", "Debit A/C","Credit A/C","Closing Balance","Type C/D"};
                    Sheet sheet = workbook.createSheet(" Sales Reg.Report");
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        if (mfgShow) {
                            ...
                            row.createCell(1).setCellValue(batchNo.get("debit").getAsString());
                            row.createCell(2).setCellValue(batchNo.get("credit").getAsString());
                            row.createCell(3).setCellValue(batchNo.get("closing_balance").getAsString());
                        ...
                        sumOfQnty += batchNo.get("credit").getAsDouble();
                    ...
                    Cell ccell = prow.createCell(2);
                    cell.setCellValue(sumOfQty);
                    ccell.setCellValue(sumOfQnty);
...
//                            System.out.println("data ------> " + s);
    ...
    public InputStream exportExcelPurchaseRegReport2(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
//            Boolean mfgShow = Boolean.valueOf(request.getParameter("mfgShow"));
            String JsonToStr = jsonRequest.get("list");
...
//                        headers = new String[]{"MONTH", "No. of Voucher","Total Reciept Amt"};
                    Sheet sheet = workbook.createSheet(" CreditNoteReport");
