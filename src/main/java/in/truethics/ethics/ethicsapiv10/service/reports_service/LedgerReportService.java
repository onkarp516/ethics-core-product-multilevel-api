        ...
        for (LedgerTransactionPostings ledgerTransactionDetails : mlist) {
            ...
            if (!ledgerTransactionDetails.getOperations().equalsIgnoreCase("Delete")) {
                ...
                if (tranx_type == 1) {
                    ...
                    if (tranxReceiptPerticulars != null && tranxReceiptPerticulars.size() > 0) {
...
//                    System.out.println("end Date:" + obj[1].toString());
            ...
            for (LedgerTransactionPostings ledgerTransactionPostings : mlist) {
                ...
                inside.addProperty("invoice_no", ledgerTransactionPostings.getInvoiceNo());
                inside.addProperty("invoice_id", ledgerTransactionPostings.getTransactionId());
                inside.addProperty("unique_code",ledgerTransactionPostings.getLedgerMaster().getUniqueCode());
                Long tranx_type = ledgerTransactionPostings.getTransactionType().getId();
                if (tranx_type == 1) {
                    TranxPurInvoice tranxPurInvoice;
                    if (users.getBranch() != null) {
...
//                        inside.addProperty("closing_balance",tranxPurInvoice.getBalance());
                ...
                inside.addProperty("voucher_type", ledgerTransactionPostings.getTransactionType().getTransactionName());
                inside.addProperty("tranx_unique_code", ledgerTransactionPostings.getTransactionType().getTransactionCode());
                LedgerMaster ledgerMaster = ledgerMasterRepository.findByIdAndStatus(ledger_master_id, true);
                inside.addProperty("ledgerName", ledgerMaster.getLedgerName());
                res.addProperty("companyName", ledgerMaster.getOutlet().getCompanyName());
                if (ledgerTransactionPostings.getLedgerType().equalsIgnoreCase("CR")) {
...
//            Users users = jwtRequestFilter.getUserDataFromToken(request.get("Authorization").substring(7));
    ...
    //API for Excel Export Ledeger Report screen - 1
        ...
        try {
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"PARTICULAR NAME", "DEBIT AMOUNT", "CREDIT AMOUNT"};
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        Row row = sheet.createRow(rowIdx++);
                            ...
                            row.createCell(1).setCellValue(batchNo.get("dr").getAsDouble());
                            row.createCell(2).setCellValue(batchNo.get("cr").getAsDouble());
    ...
    //API for Excel Export Ledger Report Screen-2
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    ...
                    for (int i = 0; i < productBatchNos.size(); i++) {
                        ...
                        row.createCell(3).setCellValue(batchNo.get("total_balance").getAsDouble());
    ...
    //API for Excel Export Ledegr Report 3
    public InputStream exportLedgerReport3(Map<String, String> jsonRequest, HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    String[] headers = {"DATE", "LEDGER NAME", "VOUCHER TYPE","VOUCHER NO.","DEBIT AMOUNT","CREDIT AMOUNT","CLOSING BALANCE","TYPE"};
...
//                        if(batchNo.keySet().contains("type"))    //It checks wheather the "type" is present or not in list
                        ...
                        if(batchNo.keySet().contains("type"))
                        row.createCell(7).setCellValue(batchNo.get("type").getAsString());
