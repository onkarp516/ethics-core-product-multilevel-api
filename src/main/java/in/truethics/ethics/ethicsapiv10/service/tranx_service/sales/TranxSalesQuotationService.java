...
//                salesQuotationDutiesTaxes = salesQuotationTransaction.getSalesQuotationDutiesTaxes();
                ...
                jsonObject.addProperty("bill_date", DateConvertUtil.convertDateToLocalDate(salesQuotationTransaction.getBillDate()).toString());
                jsonObject.addProperty("bill_no", "ABC123");
                jsonObject.addProperty("sales_acc_id", salesAccountLedger.getId());
                jsonObject.addProperty("transaction_dt", DateConvertUtil.convertDateToLocalDate(salesQuotationTransaction.getBillDate()).toString());
                ...
                jsonObject.addProperty("totalqty", salesQuotationTransaction.getTotalqty());
                jsonObject.addProperty("tranxCode", salesQuotationTransaction.getTranxCode());
    ...
    private TranxSalesQuotation saveSalesQuotationRequest(HttpServletRequest request, Users users) {
        ...
        salesQuotationTransaction.setBillDate(DateConvertUtil.convertStringToDate(request.getParameter("bill_dt")));
...
//        else salesOrderTransaction.setTransportName("NA");
        ...
        try {
            ...
            /***** this scenario is for Upahar Trading only , cakes order from client and payment made as an advance ****/
//            if (paramMap.containsKey("order_type")) {
//                if (!request.getParameter("order_type").equalsIgnoreCase("core_product")) {
//                    createReceiptInvoice(mSalesOrderTransaction, users, "create");
//                }
//            }
        } catch (DataIntegrityViolationException e) {
            e.printStackTrace();
...
//                query = query + " ORDER BY sundry_debtors_id ASC";
    ...
    private SalesQoutationDTO convertToDTDTO(TranxSalesQuotation tranxSalesQuotation) {
        ...
        salesQoutationDTO.setId(tranxSalesQuotation.getId());
        salesQoutationDTO.setBill_date(DateConvertUtil.convertDateToLocalDate(tranxSalesQuotation.getBillDate()).toString());
        salesQoutationDTO.setBill_no(tranxSalesQuotation.getSq_bill_no());
        salesQoutationDTO.setTotal_amount(tranxSalesQuotation.getTotalAmount());
        salesQoutationDTO.setTotal_base_amount(tranxSalesQuotation.getTotalBaseAmount());
        salesQoutationDTO.setNarration(tranxSalesQuotation.getNarration());
//        salesQoutationDTO.setPayment_mode(tranxSalesQuotation.gePaymentMode());
        salesQoutationDTO.setSales_quotation_status(tranxSalesQuotation.getTransactionStatus().getStatusName());
        salesQoutationDTO.setSale_account_name(tranxSalesQuotation.getSalesAccountLedger().getLedgerName());
//        salesQoutationDTO.setSale_serial_number(tranxSalesQuotation.getSalesSerialNumber());
        salesQoutationDTO.setSundry_debtors_id(tranxSalesQuotation.getSundryDebtors().getId());
        salesQoutationDTO.setSundry_debtors_name(tranxSalesQuotation.getSundryDebtors().getLedgerName());
        salesQoutationDTO.setTax_amt(tranxSalesQuotation.getTotalTax());
        salesQoutationDTO.setTaxable_amt(tranxSalesQuotation.getTaxableAmount());
        salesQoutationDTO.setTranxCode(tranxSalesQuotation.getTranxCode());
...
*/
                                    ...
                                    if (batchNos != null && batchNos.size() > 0) {
                                        ...
                                        for (ProductBatchNo mBatch : batchNos) {
                                            ...
                                            if (mBatch.getExpiryDate() != null) {
                                                if (DateConvertUtil.convertDateToLocalDate(quotationTranx.getBillDate()).isAfter(mBatch.getExpiryDate())) {
    ...
    public JsonObject getSalesQuotationByIdNew(HttpServletRequest request) {
        ...
        try {
            ...
            result.addProperty("debtor_name", tranxSalesQuotation.getSundryDebtors().getLedgerName());
            result.addProperty("narration", tranxSalesQuotation.getNarration());
            result.addProperty("gstNo", tranxSalesQuotation.getGstNumber());
    ...
    public JsonObject getQuotationBillPrint(HttpServletRequest request) {
        ...
        try {
            ...
            if (salesQuotation != null) {
                ...
                invoiceObject.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(salesQuotation.getBillDate()).toString());
