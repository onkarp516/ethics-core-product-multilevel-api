...
//                    responseObject.setResponseObject(mAppConfig.getId().toString());
    ...
    public JsonObject getOutletById(HttpServletRequest request) {
        ...
        if (mOutlet != null) {
            ...
            response.addProperty("foodLicenseNo", mOutlet.getFoodLicenseNo());
            response.addProperty("foodLicenseExpiryDate", mOutlet.getFoodLicenseExpiry() != null ?
                    mOutlet.getFoodLicenseExpiry().toString() : "");
            response.addProperty("manufacturingLicenseNo", mOutlet.getManufacturingLicenseNo());
            ...
            response.addProperty("place", mOutlet.getPlace());
            response.addProperty("route", mOutlet.getRoute());
...
*/
    ...
    /* public JsonObject getOutletsOfSuperAdmin(HttpServletRequest request) {
         ...
         JsonObject res = new JsonObject();
         res = getOutletData(outlets);
         return res;
     }*/
   /* public JsonObject getOutletsOfSuperAdmin(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        Long userId = users.getId();
        List<Users> userList = userRepository.findStatus(true);
        JsonObject res = new JsonObject();
        res = getUserData(userList);
        return res;
    }*/
    public JsonObject getOutletsOfSuperAdmin(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
