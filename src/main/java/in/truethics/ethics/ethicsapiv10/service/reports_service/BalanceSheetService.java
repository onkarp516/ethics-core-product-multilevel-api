...
//        JsonObject res = new JsonObject();
//        List<Object[]> list = new ArrayList<>();
//        List<Object[]> associatesList = new ArrayList<>();
...
//                    associatesList = postingsRepository.findByAssocPrincipleGroupTotalAmountOuletAndBranchStatusStep1(users.getOutlet().getId(), users.getBranch().getId(), true, principle_id, startDatep, endDatep);
//                } else {
...
//                    associatesList = postingsRepository.findByAssocPrincipleGroupTotalAmountOuletAndStatusStep1(users.getOutlet().getId(), true, principle_id, startDatep, endDatep);
//                }
//            } else {
//                if (users.getBranch() != null) {
//                    list = postingsRepository.findByPrincipleGroupTotalAmountOuletAndBranchStatusStep1(users.getOutlet().getId(), users.getBranch().getId(), true, principle_id, startDatep, endDatep);
...
//                ledger_master_id = Long.parseLong(objp[0].toString());
//                LedgerMaster mLedger = ledgerMasterRepository.findByIdAndStatus(ledger_master_id, true);
//                credit_total = postingsRepository.sumofAmount(ledger_master_id,users.getOutlet().getId(),true,startDatep, endDatep);
//
...
//
////                inside.addProperty("key",);
//                innerArr.add(inside);
//            }
//            /***** for associate groups *****/
////            for (int i = 0; i < associatesList.size(); i++) {
////                JsonObject inside = new JsonObject();
////                Object[] objp = associatesList.get(i);
//////                credit_total = Double.parseDouble(objp[0].toString());
////                ledger_master_id = Long.parseLong(objp[0].toString());
////                System.out.println("Ledger Id:" + ledger_master_id);
////
...
////
////                }
////                if (ledgerMaster.getPrincipleGroups() != null) {
////                    inside.addProperty("particular", ledgerMaster.getPrincipleGroups().getGroupName());
////                    inside.addProperty("principleGroup_id", ledgerMaster.getPrincipleGroups().getId());
////                } else
////                    inside.addProperty("particular", ledgerMaster.getAssociateGroups() != null ? ledgerMaster.getAssociateGroups().getAssociatesName() : "");
////                inside.addProperty("ledger_master_id", ledger_master_id);
//////                inside.addProperty("key",);
////                innerArr.add(inside);
////            }
    ...
    public Object getbalanceacstep4(HttpServletRequest request) {
        ...
        try {
            ...
            for (LedgerTransactionPostings ledgerTransactionPostings : mlist) {
                ...
                inside.addProperty("invoice_id", ledgerTransactionPostings.getTransactionId());// Invoice Id : 1 or 2
                Long tranx_type = ledgerTransactionPostings.getTransactionType().getId(); // Transactions Id : 1:Pur 3: Sales
                if (tranx_type == 1) {
                    ...
                    if (users.getBranch() != null) {
                        ...
                        tranxPurInvoice = tranxPurInvoiceRepository.findByIdAndOutletIdAndStatus(ledgerTransactionPostings.getTransactionId(), users.getOutlet().getId(), true);
                        inside.addProperty("particulars", tranxPurInvoice.getSundryCreditors().getLedgerName());
                        inside.addProperty("id", tranxPurInvoice.getId());
                    }
...
//                inside.addProperty("credit", ledgerTransactionPostings.getCredit());
    ...
    public Object getBalanceSheetAc(HttpServletRequest request) {     //New API
    JsonArray result = new JsonArray();
    JsonObject res = new JsonObject();
    LedgerTransactionDetails ledgerTransactionDetails = null;
    try {
