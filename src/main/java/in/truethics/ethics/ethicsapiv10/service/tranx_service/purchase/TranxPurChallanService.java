...
public class TranxPurChallanService {
                    ...
                    if (jsonArray != null && jsonArray.size() > 0) {
                        ...
                        if (mProduct.getIsInventory()) {
                            ...
                            closingUtility.stockPosting(outlet, branch, mTranxPurChallan.getFiscalYear().getId(), batchId,
                                    ...
                                    mTranxPurChallan.getTranxCode(), userId,"IN");
    ...
    private PurChallanDTO convertToDTDTO(TranxPurChallan purChallan) {
        ...
        purChallanDTO.setTax_amt(purChallan.getTotalTax());
            ...
            for (TranxPurChallanDetailsUnits mUnits : unitsArray) {
                ...
                if (mUnits.getProductBatchNo() != null) {
                    if (mUnits.getProductBatchNo().getExpiryDate() != null) {
                        if (DateConvertUtil.convertDateToLocalDate(mUnits.getTranxPurChallan().getInvoiceDate()).isAfter(mUnits.getProductBatchNo().getExpiryDate())) {
    ...
    public JsonObject getChallan(HttpServletRequest request) {
        ...
        try {
            ...
            result.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(purChallan.getInvoiceDate()).toString());
    ...
    public TranxPurChallan saveIntoPCEdit(HttpServletRequest request) {
        ...
        LocalDate date = DateConvertUtil.convertStringToLocalDate(request.getParameter("invoice_date"));
        Date strDt = DateConvertUtil.convertStringToDate(request.getParameter("invoice_date"));
        if (date.isEqual(DateConvertUtil.convertDateToLocalDate(tranxPurChallan.getInvoiceDate()))) {
                                    ...
                                    if (batchNos != null && batchNos.size() > 0) {
                                        for (ProductBatchNo mBatch : batchNos) {
                                            ...
                                            if (mBatch.getExpiryDate() != null) {
                                                if (DateConvertUtil.convertDateToLocalDate(invoiceTranx.getInvoiceDate()).isAfter(mBatch.getExpiryDate())) {
...
//        result.add("invoice_list", productArray);
    ...
    public JsonObject getPurchaseChallanByIdNew(HttpServletRequest request) {
        ...
        try {
            ...
            for (TranxPurChallanDetailsUnits mUnits : unitsArray) {
                ...
                unitsJsonObjects.addProperty("is_serial", mUnits.getProduct().getIsSerialNumber() != null ?
                        mUnits.getProduct().getIsSerialNumber() : false);
                row.add(unitsJsonObjects);
            }
    ...
    public Object validatePurchaseChallan(HttpServletRequest request) {
        ...
        if (purInvoice != null) {
            responseMessage.setMessage("Duplicate purchase challan number");
    ...
    public JsonObject getChallanSupplierListByProductId(HttpServletRequest request) {
        ...
        for (TranxPurChallanDetailsUnits obj : tranxPurChallanDetailsUnits) {
            ...
            response.addProperty("invoice_date", DateConvertUtil.convertDateToLocalDate(obj.getTranxPurChallan().getInvoiceDate()).toString());
    ...
    public JsonObject purchasePrintChallan(HttpServletRequest request) {
        ...
        try {
            ...
            if (purChallan != null) {
                ...
                invoiceObject.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(purChallan.getInvoiceDate()).toString());
