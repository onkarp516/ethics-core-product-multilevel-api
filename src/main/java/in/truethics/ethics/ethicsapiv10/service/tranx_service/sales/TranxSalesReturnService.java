                        ...
                        if(productRow!=null) {
                            if (mSalesTranx.getTransactionDate().compareTo(productRow.getTranxDate()) == 0 &&
                                ...
                                closingUtility.stockPosting(outlet, branch, mSalesTranx.getFiscalYear().getId(), batchId,
                                        ...
                                        mSalesTranx.getTranxCode(), id,"IN");
                            }
                        }
                    } else {
...
//                Long ledgerId = object.get("ledger").getAsLong();
    ...
    public void saveIntoSalesInvoiceDetails(JsonArray invoiceDetails, TranxSalesReturnInvoice mSalesTranx, Branch branch, Outlet outlet, Long userId, TransactionTypeMaster tranxType) {
        ...
        for (int i = 0; i < invoiceDetails.size(); i++) {
            ...
            try {
                ...
                if (mSalesTranx.getTranxSalesInvoice() != null) {
                    TranxSalesInvoiceDetailsUnits mUnits = tranxSalesInvoiceDetailsUnitRepository.
                            findBySalesInvoiceIdAndStatusAndProductId(mSalesTranx.getTranxSalesInvoice().getId(),
                                    true, mProduct.getId());
                    if (mUnits != null) {
                    ...
                    List<TranxSalesInvoiceDetailsUnits> unitDetails = tranxSalesInvoiceDetailsUnitRepository.
                            findProductTotalQty(mProduct.getId(), productBatchNo.getId(),
                                    mSalesTranx.getSundryDebtors().getId());
                    Double remaining_qty = 0.0;
                    Double qty = object.get("qty").getAsDouble();
                    for (TranxSalesInvoiceDetailsUnits mDetails : unitDetails) {
                        ...
                        tranxSalesInvoiceDetailsUnitRepository.save(mDetails);
                        if (remaining_qty <= 0)
                            break;
                    }
                }
            ...
            try {
                ...
                if (mProduct.getIsInventory() && flag) {
                    ...
                    closingUtility.stockPosting(outlet, branch, mSalesTranx.getFiscalYear().getId(), batchId,
                            ...
                            mSalesTranx.getId(), units.getId(), levelAId, levelBId, levelCId, productBatchNo,
                            mSalesTranx.getTranxCode(), userId,"IN");
    ...
    private void insertIntoTranxDetailSD(TranxSalesReturnInvoice mSalesTranx, TransactionTypeMaster
            tranxType, String crdrType, String operation) {
        try {
    ...
    private void insertIntoTranxDetailSA(TranxSalesReturnInvoice mSalesTranx, TransactionTypeMaster
            tranxType, String crdrType, String operation) {
        try {
            /**** New Postings Logic *****/
    ...
    private void insertIntoTranxDetailsSalesDiscount(TranxSalesReturnInvoice mSalesTranx, TransactionTypeMaster
            tranxType, String crdrType, String operation) {
        try {
    ...
    public void insertDB(TranxSalesReturnInvoice mSalesTranx, String ledgerName, TransactionTypeMaster
            tranxType, String crdrType, String operation) {
        try {
            /* Sale Duties Taxes */
    ...
    private void insertFromDutiesTaxes(TranxSalesReturnInvoiceDutiesTaxes mDuties, TranxSalesReturnInvoice
            mSalesTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        if (mSalesTranx.getSalesDiscountLedger() != null) {
            /**** New Postings Logic *****/
            /**** New Postings Logic *****/
    ...
    private void insertFromAdditionalCharges(TranxSalesReturnInvoiceAddCharges
                                                     mAdditinoalCharges, TranxSalesReturnInvoice mSalesTranx, TransactionTypeMaster tranxType, String
                                                     crdrType, String operation) {
        /**** New Postings Logic *****/
    ...
    private void insertIntoNewReference(TranxSalesReturnInvoice mSalesTranx, HttpServletRequest request, String key) {
        ...
        if (key.equalsIgnoreCase("create")) {
            ...
            tranxCreditNoteNewReference = tranxCreditNoteNewReferenceRepository.findByTranxSalesReturnInvoiceIdAndStatus(mSalesTranx.getId(), true);
        }
...
//            result.addProperty("transaction_dt", salesInvoice.getTransactionDate().toString());
    ...
    public JsonObject AllsalesReturnsByOutlet(HttpServletRequest request) {
        ...
        for (TranxSalesReturnInvoice invoices : salesInvoice) {
            ...
            response.addProperty("tranxCode", invoices.getTranxCode());
            result.add(response);
        }
        JsonObject output = new JsonObject();
        output.addProperty("message", "success");
    ...
    private SalesReturnDTO convertToDTDTO(TranxSalesReturnInvoice salesReturn) {
        ...
        salesReturnDTO.setTransaction_date(DateConvertUtil.convertDateToLocalDate(salesReturn.getTransactionDate()).toString());
...
//        salesReturnDTO.setInvoice_no(salesReturn.getTranxSalesInvoice().getSalesInvoiceNo());
    ...
    public JsonObject getSalesReturnByIdNew(HttpServletRequest request) {
        ...
        try {
            ...
            result.addProperty("id", salesInvoice.getId());
            result.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(salesInvoice.getTransactionDate()).toString());
            ...
            result.addProperty("transaction_dt", DateConvertUtil.convertDateToLocalDate(salesInvoice.getTransactionDate()).toString());
...
//            if(object.has("amt")){
               Double  amount = object.get("amt").getAsDouble();
//            }else amount=0.0;
//             if(object.has("ledgerId")){
            if(object.has("ledgerId") && object.get("ledgerId").getAsString() != ""){
                Long   ledgerId = object.get("ledgerId").getAsLong();
...
//            Long detailsId = object.get("additional_charges_details_id").getAsLong();
    ...
    public JsonObject getSalesReturnBillPrint(HttpServletRequest request) {
        ...
        try {
            ...
            if (salesInvoice != null) {
                ...
                invoiceObject.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(salesInvoice.getTransactionDate()).toString());
    ...
    public JsonObject validatesaleReturnProducts(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
        Long sundryDebtorsId = Long.parseLong(request.getParameter("supplier_id"));
...
//        Long productId = Long.parseLong(request.getParameter("product_id")  != null && !request.getParameter("product_id").isEmpty() ?request.getParameter("product_id")  : "" );
        ...
        List<TranxSalesInvoiceDetailsUnits> productList = new ArrayList<>();
        productList = tranxSalesInvoiceDetailsUnitRepository.findProductList(sundryDebtorsId, productId, true);

        if (productList != null && productList.size() > 0) {
            int cnt = 0;
            JsonObject responseObj = new JsonObject();
            for (TranxSalesInvoiceDetailsUnits mDetails : productList) {
    ...
    public JsonObject validateSalesReturnProductBatch(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
...
//        JsonArray result = new JsonArray();
        ...
        batchRowId = tranxSalesInvoiceDetailsUnitRepository.findBatchList(productId, batchId, sundryDebtorsId, true);
...
//                response.add("data", result);
    ...
    public JsonObject validateSalesReturnProductQuantity(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
...
//        JsonArray result = new JsonArray();
        ...
        Double qty = 0.0;
        qty = tranxSalesInvoiceDetailsUnitRepository.findQuantity(productId, batchId, rate, true, sundryDebtorsId);
//
//        if (qty != null && qty<=quantity) {

        response.addProperty("quantity", qty);
