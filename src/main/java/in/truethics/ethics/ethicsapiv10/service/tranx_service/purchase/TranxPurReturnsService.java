...
public class TranxPurReturnsService {
    ...
    public Object createPurReturnsInvoices(HttpServletRequest request) {
        ...
        if (mPurchaseTranx != null) {
            ...
            if (request.getParameter("paymentMode").equalsIgnoreCase("credit"))
                insertIntoNewReference(mPurchaseTranx, request, "create");
            else {
                ...
                for (JsonElement mElement : array) {
                    ...
                    if (mObject.has("invoice_id"))
                        invoiceId = mObject.get("invoice_id").getAsLong();
                    if (invoiceId > 0) {
                        mInvoice = tranxPurInvoiceRepository.findByIdAndStatus(
                                invoiceId, true);
    ...
    /*** creating debit note (creating new reference automatically) while adjusting
     return amount ***/
    private void insertIntoNewReference(TranxPurReturnInvoice mPurchaseTranx, HttpServletRequest request, String key) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        ...
        try {
            ...
            if (key.equalsIgnoreCase("create")) {
                ...
                mDetails = tranxDebitNoteDetailsRepository.findByStatusAndTranxDebitNoteMasterId(
                        true, newDebitNote.getId());
            }
    ...
    public void saveIntoPurchaseInvoiceDetails(JsonArray array,
                                               TranxPurReturnInvoice mPurchaseTranx, Branch branch,
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            try {
                  ...
                  /*  if (outlet.getGstTypeMaster().getId() == 1L) { //Registered
                        ...
                        if (object.get("b_details_id").getAsLong() == 0) {
                            ...
                            if (object.has("manufacturing_date") &&
                                    !object.get("manufacturing_date").getAsString().equalsIgnoreCase("") &&
                                    !object.get("manufacturing_date").getAsString().toLowerCase().contains("invalid"))
                                productBatchNo.setManufacturingDate(LocalDate.parse(
                                        object.get("manufacturing_date").getAsString()));
                            if (object.has("rate_a") && !object.get("rate_a").getAsString().equalsIgnoreCase(""))
                ...
                /*** Update Return Qty *****/
                if (mPurchaseTranx.getTranxPurInvoice() != null) {
                    TranxPurInvoiceDetailsUnits mUnits = tranxPurInvoiceUnitsRepository.
                            findByPurchaseTransactionIdAndStatusAndProductId(mPurchaseTranx.getTranxPurInvoice().getId(),
                                    true, mProduct.getId());
                    if (mUnits != null) {
                    ...
                    List<TranxPurInvoiceDetailsUnits> unitDetails = tranxPurInvoiceDetailsUnitsRepository.
                            findProductTotalQty(mProduct.getId(), productBatchNo.getId(),
                                    mPurchaseTranx.getSundryCreditors().getId());
                    Double remaining_qty = 0.0;
                    Double qty = object.get("qty").getAsDouble();
                    for (TranxPurInvoiceDetailsUnits mDetails : unitDetails) {
                        Double total_qty = mDetails.getQty() - mDetails.getReturnQty();
                        if (total_qty == 0.0) continue;
                        if (total_qty <= qty) {
                            mDetails.setReturnQty(total_qty);
                            remaining_qty = remaining_qty + (qty - total_qty);
                            qty = remaining_qty;
                        } else {
                            mDetails.setReturnQty(mDetails.getReturnQty() + qty);
                            remaining_qty = remaining_qty + (qty - total_qty);
                        }
                        tranxPurInvoiceDetailsUnitsRepository.save(mDetails);
                        if (remaining_qty <= 0)
                            break;
                    }
                }
    ...
    /*    String startYear = generateFiscalYear.getStartYear();
     ...
     /*   String csCode = companyName.toUpperCase() + "-" + startYear + endYear
        ...
        String csCode = "PRSRT" + currentMonth + serailNo;
    ...
    public void insertIntoTranxDetailSC(TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        try {
            ...
            /**** New Postings Logic *****/
//            ledgerCommonPostings.callToPostings(mPurchaseTranx.getTotalAmount(), mPurchaseTranx.getSundryCreditors(), tranxType, mPurchaseTranx.getSundryCreditors().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);
            ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getTotalAmount(), mPurchaseTranx.getSundryCreditors(), tranxType, mPurchaseTranx.getSundryCreditors().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation,mPurchaseTranx.getTranxCode());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void insertIntoTranxDetailPA(TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        /**** New Postings Logic *****/
//        ledgerCommonPostings.callToPostings(mPurchaseTranx.getTotalBaseAmount(), mPurchaseTranx.getPurchaseAccountLedger(), tranxType, mPurchaseTranx.getPurchaseAccountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);
        ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getTotalBaseAmount(), mPurchaseTranx.getPurchaseAccountLedger(), tranxType, mPurchaseTranx.getPurchaseAccountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation,mPurchaseTranx.getTranxCode());
    ...
    public void insertIntoTranxDetailPD(TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        try {
            if (mPurchaseTranx.getPurchaseDiscountLedger() != null) {
                /**** New Postings Logic *****/
                /*ledgerCommonPostings.callToPostings(mPurchaseTranx.getTotalPurchaseDiscountAmt(), mPurchaseTranx.getPurchaseDiscountLedger(), tranxType, mPurchaseTranx.getPurchaseDiscountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);*/
                ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getTotalPurchaseDiscountAmt(), mPurchaseTranx.getPurchaseDiscountLedger(), tranxType, mPurchaseTranx.getPurchaseDiscountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation,mPurchaseTranx.getTranxCode());
    ...
    private void insertIntoTranxDetailRO(TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType) {
        if (mPurchaseTranx.getRoundOff() >= 0) {
            /**** New Postings Logic *****/
/*
            ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(), tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "CR", true, "Purchase Return", "Insert");
*/
            ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(), tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "CR", true, "Purchase Return", "Insert",mPurchaseTranx.getTranxCode());

        } else if (mPurchaseTranx.getRoundOff() < 0) {
            /**** New Postings Logic *****/
/*
            ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(), tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "DR", true, "Purchase Return", "Insert");
*/
            ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(), tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "DR", true, "Purchase Return", "Insert",mPurchaseTranx.getTranxCode());
    ...
    public void insertFromDutiesTaxes(TranxPurReturnInvoiceDutiesTaxes mDuties, TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        ...
        /**** New Postings Logic *****/
//        ledgerCommonPostings.callToPostings(mDuties.getAmount(), mDuties.getDutiesTaxes(), tranxType, mDuties.getDutiesTaxes().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);
        ledgerCommonPostings.callToPostingsTranxCode(mDuties.getAmount(), mDuties.getDutiesTaxes(), tranxType, mDuties.getDutiesTaxes().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation,mPurchaseTranx.getTranxCode());
    ...
    public void insertFromAdditionalCharges(TranxPurReturnInvoiceAddCharges mAdditinoalCharges, TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        /**** New Postings Logic *****/
//        ledgerCommonPostings.callToPostings(mAdditinoalCharges.getAmount(), mAdditinoalCharges.getAdditionalCharges(), tranxType, mAdditinoalCharges.getAdditionalCharges().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getTransactionDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);
        ledgerCommonPostings.callToPostingsTranxCode(mAdditinoalCharges.getAmount(), mAdditinoalCharges.getAdditionalCharges(), tranxType, mAdditinoalCharges.getAdditionalCharges().getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation,mPurchaseTranx.getTranxCode());
    ...
    public JsonObject purchaseListSupplierWise(HttpServletRequest request) {
        ...
        if (paramMap.containsKey("dateFrom") && paramMap.containsKey("dateTo")) {
            if (users.getBranch() != null) {
                purInvoice = tranxPurInvoiceRepository.findByBranchSuppliersWithDates(users.getOutlet().getId(),
            ...
            if (users.getBranch() != null) {
                purInvoice = tranxPurInvoiceRepository.findByOutletIdAndBranchIdAndStatusAndSundryCreditorsId(
                        users.getOutlet().getId(), users.getBranch().getId(), true, ledgerId);
            } else {
                purInvoice = tranxPurInvoiceRepository.findByOutletIdAndStatusAndSundryCreditorsIdAndBranchIsNull(
        ...
        try {
            ...
            for (LedgerTransactionPostings mPostings : mInoiceLedgers) {
                try {
                    ...
                    purchaseReturnLogger.error("Exception in Delete functionality for all ledgers of " +
    ...
    public JsonObject purReturnsByOutlet(HttpServletRequest request) {
        ...
        for (TranxPurReturnInvoice invoices : purInvoice) {
            ...
            response.addProperty("transaction_date", invoices.getTransactionDate().toString());
            response.addProperty("purchase_return_date", invoices.getPurReturnDate() != null ? DateConvertUtil.convertDateToLocalDate(invoices.getPurReturnDate()).toString() : "");
    ...
    public Object purReturnsByOutlet(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        try {
            ...
            GenericDatatable<PurReturnDTO> data = new GenericDatatable<>(purReturnDTOList, purchaseArrayList.size(),
...
/*                    unitDetails.forEach(mUnit -> {
        ...
        if (mPurchaseTranx != null) {
            if (request.getParameter("paymentMode").equalsIgnoreCase("credit"))
                ...
                for (JsonElement mElement : array) {
                    ...
                    mBills = tranxPurReturnAdjBillsRepository.
                            findByIdAndStatus(mObject.get("id").getAsLong(), true);
    ...
    private TranxPurReturnInvoice saveIntoPurchaseReturnEdit(HttpServletRequest request) {
        ...
        LocalDate mDate = LocalDate.now();
        invoiceTranx.setTransactionDate(mDate);
        LocalDate returnDate = DateConvertUtil.convertStringToLocalDate(request.getParameter("purchase_return_date"));
...
//            Long detailsId = object.get("additional_charges_details_id").getAsLong();
    ...
    private void saveIntoPurchaseInvoiceDetailsEdit(JsonArray array, TranxPurReturnInvoice mPurchaseTranx, Branch branch, Outlet outlet, Long id, TransactionTypeMaster tranxType, HttpServletRequest request, String rowsDeleted) {
        List<TranxPurReturnInvoiceProductSrNo> newSerialNumbers = new ArrayList<>();
        /* Purchase Product Details Start here */
        Double tranxQty = 0.0;
        TransactionStatus status = transactionStatusRepository.findByIdAndStatus(2L, true);
...
//                            mproductBatchNo.setMinMargin(object.get("margin_per").getAsDouble());
               ...
               /* if (mProduct.getIsInventory() == false && mProduct.getIsBatchNumber() == false) {
                ...
                if (mProduct.getIsInventory() && flag) {
                    ...
                    if (detailsId != 0) {
                        ...
                        System.out.println("invoice Date"+mPurchaseTranx.getPurReturnDate());
                        System.out.println("tranx De"+productRow.getTranxDate());
                        System.out.println("qty "+ invoiceUnits.getQty());
                        System.out.println("Tranx qty "+  tranxQty);
                        if(productRow!=null) {
                            if (mPurchaseTranx.getPurReturnDate().compareTo(productRow.getTranxDate()) == 0 &&
                                ...
                                closingUtility.stockPosting(outlet, branch, mPurchaseTranx.getFiscalYear().getId(), batchId,
                                        ...
                                        mPurchaseTranx.getId(), units.getId(), levelAId, levelBId, levelCId, productBatchNo,
                                        mPurchaseTranx.getTranxCode(), id,"OUT");
                            }
                        }
                    } else {
                        inventoryCommonPostings.callToInventoryPostings("DR",
                                mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(),
                                object.get("qty").getAsDouble() + free_qty, branch, outlet, mProduct, tranxType,
                                levelA, levelB, levelC, units, productBatchNo, batchNo, mPurchaseTranx.getFiscalYear(), serialNo);
                    }
                    /***** End of new architecture of Inventory Postings *****/
                }
            } catch (Exception e) {
                        ...
                        if (deletedRowsId.has("del_id")) {
                            ...
                            if (mDeletedInvoices != null) {
                                ...
                                try {
                                    ...
                                    inventoryCommonPostings.callToInventoryPostings("DR",
                                            mDeletedInvoices.getTranxPurReturnInvoice().getPurReturnDate(),
                                            mDeletedInvoices.getTranxPurReturnInvoice().getId(),
                                            ...
                                            mDeletedInvoices.getProductBatchNo().getBatchNo(),
                                            mDeletedInvoices.getTranxPurReturnInvoice().getFiscalYear(),
                                            null);
                                } catch (Exception e) {
                                    e.printStackTrace();
    ...
    public Object editPurchaseReturns(HttpServletRequest request) {
       ...
       /* this parameter segregates whether pur return is from purchase invoice
        ...
        invoiceTranx.setOperations("insert");
        if (paramMap.containsKey("narration")) invoiceTranx.setNarration(request.getParameter("narration"));
        else invoiceTranx.setNarration("NA");
        invoiceTranx.setPurReturnDate(strDt);
        if (paramMap.containsKey("gstNo")) {
            if (!request.getParameter("gstNo").equalsIgnoreCase("")) {
                invoiceTranx.setGstNumber(request.getParameter("gstNo"));
    ...
    public JsonObject getPurchaseReturnByIdNew(HttpServletRequest request) {
        ...
        try {
            ...
            for (TranxPurReturnDetailsUnits mUnits : unitsArray) {
                ...
                if (mUnits.getProductBatchNo() != null) {
                    if (mUnits.getProductBatchNo().getExpiryDate() != null) {
                        if (DateConvertUtil.convertDateToLocalDate(purchaseInvoice.getPurReturnDate()).isAfter(mUnits.getProductBatchNo().getExpiryDate())) {
                            unitsJsonObjects.addProperty("is_expired", true);
                        } else {
                            unitsJsonObjects.addProperty("is_expired", false);
                        }
...
////                    json_charges.addProperty("percent", mAdditionalCharges.get);
//                    jsonAdditionalList.add(json_charges);
//                }
//            }
    ...
    public JsonObject getPurchaseReturnSupplierListByProductId(HttpServletRequest request) {
        ...
        for (TranxPurReturnDetailsUnits obj : tranxPurReturnDetailsUnits) {
            ...
            response.addProperty("invoice_date", DateConvertUtil.convertDateToLocalDate(obj.getTranxPurReturnInvoice().getPurReturnDate()).toString());
...
//        Map<String, String[]> paramMap = request.get();
                ...
                if (ledgerMaster.getBalancingMethod().getId() == 1) {
                    ...
                    if (purInvoice.size() > 0) {
                        for (TranxPurInvoice newPurInvoice : purInvoice) {
                            ...
                            response.addProperty("invoice_date", DateConvertUtil.convertDateToLocalDate(newPurInvoice.getInvoiceDate()).toString());
    ...
    public Object validatePurchaseReturnUpdate(HttpServletRequest request) {
        ...
        if (users.getBranch() != null) {
            purReturn = tranxPurReturnsRepository.findByOutletIdAndBranchIdAndSundryCreditorsIdAndPurRtnNoIgnoreCase(
                    users.getOutlet().getId(), users.getBranch().getId(), sundryCreditorId, request.getParameter("bill_no"));
        } else {
            purReturn = tranxPurReturnsRepository.findByOutletIdAndSundryCreditorsIdAndPurRtnNoIgnoreCaseAndBranchIsNull(
                    users.getOutlet().getId(), sundryCreditorId, request.getParameter("bill_no"));
        }
    ...
    public JsonObject purchasePrintReturn(HttpServletRequest request) {
        ...
        try {
            ...
            if (purReturn != null) {
                ...
                invoiceObject.addProperty("invoice_dt", DateConvertUtil.convertDateToLocalDate(purReturn.getPurReturnDate()).toString());
                ...
                unitDetails.forEach(mUnit -> {
                    ...
                    mObject.addProperty("mfg_date", mUnit.getProductBatchNo() != null ? (mUnit.getProductBatchNo().getManufacturingDate() != null ?
                            mUnit.getProductBatchNo().getManufacturingDate().toString() : "") : "");
                    mObject.addProperty("exp_date", mUnit.getProductBatchNo() != null ? (mUnit.getProductBatchNo().getExpiryDate() != null ?
                            mUnit.getProductBatchNo().getExpiryDate().toString() : "") : "");
...
//        return output;
//    }
    public JsonObject validatePurchaseReturnProducts(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
        Long sundryCreditorId = Long.parseLong(request.getParameter("supplier_id"));
        Long productId = Long.parseLong(request.getParameter("product_id"));
//        Long productId = Long.parseLong(request.getParameter("product_id")  != null && !request.getParameter("product_id").isEmpty() ?request.getParameter("product_id")  : "" );

        JsonObject response = new JsonObject();
        JsonArray result = new JsonArray();

        List<TranxPurInvoiceDetailsUnits> productList = new ArrayList<>();
        productList = tranxPurInvoiceDetailsUnitsRepository.findProductList(sundryCreditorId, productId, true);

        if (productList != null && productList.size() > 0) {
            int cnt = 0;
            JsonObject responseObj = new JsonObject();
            for (TranxPurInvoiceDetailsUnits mDetails : productList) {
                if (mDetails.getProduct().getId() == productId) {
                    responseObj.addProperty("product_id", mDetails.getProduct().getId());
                    responseObj.addProperty("product_name", mDetails.getProduct().getProductName());
                    responseObj.addProperty("product_rate", mDetails.getRate());
                    responseObj.addProperty("product_qty", mDetails.getQty());
                    responseObj.addProperty("product_batch", mDetails.getProductBatchNo().getBatchNo());
            ...
            if (cnt == 0) {
                ...
                response.addProperty("message", "product found");
    ...
    public JsonObject validatePurchaseReturnProductBatch(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
...
//        JsonArray result = new JsonArray();
        ...
        List<TranxPurInvoiceDetailsUnits> batchList = new ArrayList<>();
        List<Long> batchRowId = null;
        batchRowId = tranxPurInvoiceDetailsUnitsRepository.findBatchList(productId, batchId, sundryCreditorId, true);

        if (batchRowId != null && batchRowId.size() > 0) {
            response.addProperty("message", "Batch found");
            response.addProperty("responseStatus", HttpStatus.OK.value());
        } else {
//                response.add("data", result);
            response.addProperty("message", "Batch not found");
            response.addProperty("responseStatus", HttpStatus.CONFLICT.value());
        }
        return response;
    }

    public JsonObject validatePurchaseReturnProductQuantity(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
//        ResponseMessage responseMessage = new ResponseMessage();
        Map<String, String[]> paramMap = request.getParameterMap();
        List<TranxPurInvoice> purInvoices = null;
...
//        JsonArray result = new JsonArray();
        ...
        String qty = "";
        qty = tranxPurInvoiceDetailsUnitsRepository.findQuantity(productId, batchId, rate, true, sundryCreditorId);
        String qty_in_digit[] = qty.split(",");
        Double total_qty = Double.parseDouble(qty_in_digit[0]) - Double.parseDouble(qty_in_digit[1]);
//
//        if (qty != null && qty<=quantity) {

        response.addProperty("quantity", total_qty);
        response.addProperty("message", "Successful");
        response.addProperty("responseStatus", HttpStatus.OK.value());
//        }
//             else {
////                response.add("data", result);
//                response.addProperty("message", "qty not allowed");
//                response.addProperty("responseStatus",HttpStatus.CONFLICT.value() );
//            }
