...
public class LedgerReportController {
   ...
   /* @PostMapping(path = "/get_profit_and_loss_ac")
    ...
    //API for Ledger Report screen-1
    ...
    //API for Ledger Report screen-2
    ...
    //API for Ledger Report -3
    @PostMapping(path = "/exportExcelLedgerReport3")
    public Object exportLedgerReport3(@RequestBody Map<String, String> jsonRequest, HttpServletRequest request, HttpServletResponse response) {
        String filename = "excelLedgerReport1.xlsx";
        InputStreamResource file = new InputStreamResource(ledgerReportService.exportLedgerReport3(jsonRequest, request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + filename)
                .contentType(MediaType.parseMediaType("application/vnd.ms-excel"))
                .body(file);
    }
