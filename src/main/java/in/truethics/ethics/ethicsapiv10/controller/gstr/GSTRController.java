...
public class GSTRController {
    ...
    //  ExportExcel  GSTR1 Credit note Registered Api
    @PostMapping(path = "/excel_export_GSTR1_CreditNote_Reg")
    public Object exportExcelGSTR1CRNoteRegData(@RequestBody Map<String, String> jsonRequest, HttpServletRequest request, HttpServletResponse response) {
        String filename = "GSTR1_CreditNote_Reg_ExcelSheet.xlsx";
        InputStreamResource file = new InputStreamResource(service.exportExcelGSTR1CRNoteRegData(jsonRequest, request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + filename)
                .contentType(MediaType.parseMediaType("application/vnd.ms-excel"))
                .body(file);
    }
    ...
    //  Export Excel GSTR1 Credit note UnRegistered Api
    @PostMapping(path = "/excel_export_GSTR1_CreditNote_unReg")
    public Object exportExcelGSTR1CRNoteUnregData(@RequestBody Map<String, String> jsonRequest, HttpServletRequest request, HttpServletResponse response) {
        String filename = "GSTR1_CreditNote_UnReg_ExcelSheet.xlsx";
        InputStreamResource file = new InputStreamResource(service.exportExcelGSTR1CRNoteUnregData(jsonRequest, request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + filename)
                .contentType(MediaType.parseMediaType("application/vnd.ms-excel"))
                .body(file);
    }
...
//    GSTR1 Nil-rated Api
    ...
    //   ExportExcel Api for GSTR1 HSN screen1 outward data
    @PostMapping(path = "/excel_export_GSTR1_hsn_data")
    public Object excelExportGSTR1HsnData(@RequestBody Map<String, String> jsonRequest, HttpServletRequest request, HttpServletResponse response) {
        String filename = "GSTR1_hsn_ExcelSheet.xlsx";
        InputStreamResource file = new InputStreamResource(service.excelExportGSTR1HsnData(jsonRequest, request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + filename)
                .contentType(MediaType.parseMediaType("application/vnd.ms-excel"))
                .body(file);
    }

    //   ExportExcel Api for GSTR1 HSN screen2 outward data
    @PostMapping(path = "/excel_export_GSTR1_hsn2_data")
    public Object excelExportGSTR1Hsn2Data(@RequestBody Map<String, String> jsonRequest, HttpServletRequest request, HttpServletResponse response) {
        String filename = "GSTR1_hsn_ExcelSheet.xlsx";
        InputStreamResource file = new InputStreamResource(service.excelExportGSTR1Hsn2Data(jsonRequest, request));
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=" + filename)
                .contentType(MediaType.parseMediaType("application/vnd.ms-excel"))
                .body(file);
    }
