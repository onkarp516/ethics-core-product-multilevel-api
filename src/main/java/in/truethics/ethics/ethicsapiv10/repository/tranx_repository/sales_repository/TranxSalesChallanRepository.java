...
public interface TranxSalesChallanRepository extends JpaRepository<TranxSalesChallan, Long> {
    ...
    @Query(
            value = "SELECT * FROM `tranx_sales_challan_tbl` WHERE outlet_id=?1 AND bill_date BETWEEN ?2 AND ?3 AND status=?4 ORDER by bill_date DESC", nativeQuery = true)
    List<TranxSalesChallan> findSaleChallanListWithDate(Long id, LocalDate startDatep, LocalDate endDatep, boolean b);

    @Query(
            value = "SELECT * FROM `tranx_sales_challan_tbl` WHERE outlet_id=?1 AND branch_id=?2 AND bill_date BETWEEN ?3 AND ?4 AND status=?5 ORDER by bill_date DESC", nativeQuery = true)
    ...
    @Query(
            value = "SELECT * FROM `tranx_sales_challan_tbl` WHERE outlet_id=?1 AND branch_id=?2 AND " +
                    "bill_date BETWEEN ?3 AND ?4 ORDER by bill_date",
            nativeQuery = true)
    ...
    @Query(
            value = "SELECT * FROM `tranx_sales_challan_tbl` WHERE outlet_id=?1 AND " +
                    "bill_date BETWEEN ?2 AND ?3 AND branch_id IS NULL ORDER by bill_date",
            nativeQuery = true)
    ...
    @Query(
            value = "SELECT IFNULL(SUM(total_amount),0.0) FROM " +
                    "`tranx_sales_Challan_tbl` WHERE outlet_id=?1 AND branch_id=?2 AND bill_date " +
    ...
    @Query(
            value = "SELECT IFNULL(SUM(total_amount),0.0) FROM " +
                    "`tranx_sales_challan_tbl` WHERE outlet_id=?1 AND branch_id IS NULL AND bill_date " +
