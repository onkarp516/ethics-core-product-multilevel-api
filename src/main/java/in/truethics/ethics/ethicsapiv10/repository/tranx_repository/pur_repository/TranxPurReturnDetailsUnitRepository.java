...
public interface TranxPurReturnDetailsUnitRepository extends JpaRepository<TranxPurReturnDetailsUnits, Long> {
    ...
    TranxPurReturnDetailsUnits findByIdAndStatus(Long details_id, boolean b);
...
*/
    ...
    @Query(
            value = " SELECT IFNULL(avg(rate),0.0) FROM tranx_purchase_return_details_units_tbl WHERE product_id=?1 and unit_id=?2", nativeQuery = true
    )
