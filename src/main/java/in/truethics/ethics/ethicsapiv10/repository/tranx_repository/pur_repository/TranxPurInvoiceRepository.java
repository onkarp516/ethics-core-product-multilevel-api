...
*/
    ...
    @Query(
            value = "SELECT IFNULL(SUM(total_amount),0.0),IFNULL(SUM(total_base_amount),0.0),IFNULL(SUM(total_tax),0.0),COUNT(*) FROM `tranx_purchase_invoice_tbl` WHERE sundry_creditors_id=?1 AND invoice_date BETWEEN ?2 AND ?3 AND status=?4", nativeQuery = true)
    ...
    @Query(
            value = "SELECT IFNULL(SUM(balance),0.0),COUNT(*) FROM `tranx_purchase_invoice_tbl` WHERE sundry_creditors_id=?1 AND DATE(invoice_date) BETWEEN ?2 AND ?3 AND balance>0 AND status=?4", nativeQuery = true)
    ...
    List<TranxPurInvoice> findBySundryCreditorsIdAndStatus(Long ledgerId,  boolean b);
}
