...
public interface TranxReceiptPerticularsRepository extends JpaRepository<TranxReceiptPerticulars, Long> {
    ...
    @Query(
            value = "SELECT IFNULL(SUM(cr),0.0),COUNT(*) FROM `tranx_receipt_perticulars_tbl` WHERE type=?1 AND " +
                    "DATE(transaction_date) BETWEEN ?2 AND ?3 AND status=?4 AND ledger_id=?5", nativeQuery = true)
    String findmobileReceiptTotalAmt(String type, LocalDate startDatep, LocalDate endDatep, boolean b,Long ledgerId);

@Query(
        value = "SELECT * FROM core_product_multilevel_db.tranx_receipt_perticulars_tbl where type=?1 AND status=?2 AND " +
                "transaction_date between ?3 AND ?4 group by ledger_id",nativeQuery = true
)
    List<TranxReceiptPerticulars> findByTypeAndStatus(String cr, boolean b,  LocalDate startDatep, LocalDate endDatep);
