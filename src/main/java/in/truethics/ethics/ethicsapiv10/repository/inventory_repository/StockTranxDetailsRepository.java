...
public interface StockTranxDetailsRepository extends JpaRepository<InventorySummaryTransactionDetails, Long> {
    ...
    String findRow(Long tranxId, Long productId, Long tranxTypeId, LocalDate tranxDate);
    ...
    @Query(value = "SELECT * FROM stock_opening_closing_snap_tbl WHERE " +
            "product_id=?1 AND tranx_type_id=?2 AND tranx_date>?3 AND status=?4 " +
            "AND financial_year=?5 ORDER BY tranx_date ASC", nativeQuery = true)
    List<InventorySummaryTransactionDetails> findList(Long productId, Long tranxTypeId, LocalDate purDate,
                                                      Boolean status,Long fiscalYearId);
