...
public interface LedgerTransactionPostingsRepository extends JpaRepository<LedgerTransactionPostings, Long> {
    ...
    @Query(
            value = "SELECT SUM(amount),ledger_master_id ,ledger_type  FROM `ledger_transaction_postings_tbl` LEFT JOIN ledger_master_tbl ON " + " ledger_transaction_postings_tbl.ledger_master_id=ledger_master_tbl.id WHERE" + " ledger_transaction_postings_tbl.outlet_id=?1 AND" + " ledger_transaction_postings_tbl.branch_id=?2 " +
                    " AND ledger_transaction_postings_tbl.status=?3 AND principle_id=?4 AND transaction_date BETWEEN ?5 AND ?6 GROUP BY ledger_master_tbl.principle_id", nativeQuery = true
    )
    ...
    List<Object[]> findByAssocPrincipleGroupTotalAmountOuletAndBranchStatusStep1(Long id, Long id1, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);

    @Query(
            value = "SELECT ledger_master_id ,ledger_type FROM `ledger_transaction_postings_tbl` LEFT JOIN ledger_master_tbl ON " + " ledger_transaction_postings_tbl.ledger_master_id=ledger_master_tbl.id WHERE" + " ledger_transaction_postings_tbl.outlet_id=?1 AND " +
                    "ledger_transaction_postings_tbl.status=?2 AND principle_id=?3 AND" +
                    " transaction_date BETWEEN ?4 AND ?5 AND ledger_transaction_postings_tbl.branch_id IS NULL ", nativeQuery = true
    )
    ...
    @Query(
            value = "SELECT SUM(amount) FROM `ledger_transaction_postings_tbl` WHERE ledger_transaction_postings_tbl.ledger_master_id=?1 AND ledger_transaction_postings_tbl.outlet_id=?2 AND " +
                    "ledger_transaction_postings_tbl.status=?3 AND" +
                    " transaction_date BETWEEN ?4 AND ?5 AND ledger_transaction_postings_tbl.branch_id IS NULL ", nativeQuery = true
    )
    Double sumofAmount(Long ledgerid,Long id, boolean b, LocalDate startDate, LocalDate endDate);
    @Query(
            value = "SELECT ledger_master_id ,ledger_type FROM `ledger_transaction_postings_tbl` LEFT JOIN ledger_master_tbl ON " + " ledger_transaction_postings_tbl.ledger_master_id=ledger_master_tbl.id WHERE" + " ledger_transaction_postings_tbl.outlet_id=?1 AND " +
                    "ledger_transaction_postings_tbl.status=?2 AND principle_id=?3 AND" +
                    " transaction_date BETWEEN ?4 AND ?5 And ledger_transaction_postings_tbl.branch_id IS NULL GROUP BY ledger_master_tbl.principle_id", nativeQuery = true
    )
    List<Object[]> findByAssocPrincipleGroupTotalAmountOuletAndStatusStep1(Long id, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);
    ...
    @Query(
            value = "SELECT * FROM `ledger_transaction_postings_tbl` WHERE outlet_id=?1 AND " +
                    ...
                    "AND branch_id IS NULL AND status=1 ORDER by transaction_date",
            nativeQuery = true)
    ...
    @Query(
            value = "SELECT * FROM ledger_transaction_postings_tbl WHERE " +
                    "product_id=?1 and transaction_date between ?2 AND ?3 AND status=?4", nativeQuery = true
    )
    List<LedgerTransactionPostings> findProductTransactions(Long productId, LocalDate startDate, LocalDate endDate,
                                                              Boolean status);
    ...
    List<LedgerTransactionPostings> findByTranxCode(String tranx_code);
     ...
     @Query(
            value =
                    " SELECT total_amount FROM inventory_details_postings_tbl LEFT JOIN" +
                            " product_tbl ON product_tbl.id = inventory_details_postings_tbl.product_id" +
                            " RIGHT JOIN tranx_purchase_invoice_tbl ON inventory_details_postings_tbl.tranx_id = tranx_purchase_invoice_tbl.id" +
                            " WHERE inventory_details_postings_tbl.transaction_type_id=?1 AND product_tbl.tax_type=?2 ",nativeQuery = true



//           "  SELECT tranx_purchase_invoice_details_units_tbl.id,tranx_purchase_invoice_details_units_tbl.total_amount FROM"+
//             "tranx_purchase_invoice_details_units_tbl LEFT JOIN product_tbl ON"+
//            " product_tbl.id = tranx_purchase_invoice_details_units_tbl.product_id LEFT JOIN tranx_purchase_invoice_tbl ON"+
//            " tranx_purchase_invoice_tbl.id=tranx_purchase_invoice_details_units_tbl.purchase_invoice_id WHERE"+
//          " product_tbl.tax_type=?1 AND tranx_purchase_invoice_tbl.sundry_creditors_id=?2 ",
    )
    List<String>  findInCount( long id ,String taxType);
