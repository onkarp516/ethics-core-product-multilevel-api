...
//            SELECT SUM(qty) FROM `inventory_details_postings_tbl` WHERE DATE_SUB('2023-01-13', INTERVAL 1 DAY);
    ...
    @Query(
            value = "SELECT IFNULL(SUM(qty),0.0) from `inventory_details_postings_tbl` WHERE product_id=?1 AND " +
                    "tranx_action=?2 AND units_id=?3 AND transaction_type_id=?4 AND tranx_date BETWEEN ?5 ANd ?6  ",
            nativeQuery = true
    )
    Double findByTotalQty(Long productId, String crdr, Long id, Long transactionId, String toString, String toString1);
