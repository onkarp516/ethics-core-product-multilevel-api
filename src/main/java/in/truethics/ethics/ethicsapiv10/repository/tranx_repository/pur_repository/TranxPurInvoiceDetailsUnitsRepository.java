 ...
 */
    ...
    TranxPurInvoiceDetailsUnits findByStatusAndPurchaseTransactionId(boolean b, Long id);
    ...
    @Query(
            value = "SELECT * FROM tranx_purchase_invoice_details_units_tbl LEFT JOIN tranx_purchase_invoice_tbl ON " +
                    "tranx_purchase_invoice_details_units_tbl.purchase_invoice_id=tranx_purchase_invoice_tbl.id WHERE " +
                    "product_id=?2 AND tranx_purchase_invoice_tbl.sundry_creditors_id=?1 AND tranx_purchase_invoice_details_units_tbl.status=?3 AND return_qty<qty", nativeQuery = true
    )
    List<TranxPurInvoiceDetailsUnits> findProductList(Long sundryCreditorId, Long productId, boolean b);

    @Query(
            value = "SELECT tranx_purchase_invoice_details_units_tbl.id FROM `tranx_purchase_invoice_details_units_tbl` LEFT JOIN tranx_purchase_invoice_tbl ON" +
                    " tranx_purchase_invoice_details_units_tbl.purchase_invoice_id=tranx_purchase_invoice_tbl.id" +
                    " WHERE product_id=?1 AND batch_id=?2 AND tranx_purchase_invoice_tbl.sundry_creditors_id=?3 AND tranx_purchase_invoice_details_units_tbl.status=?4",
            nativeQuery = true
    )
    List<Long> findBatchList(Long productId, Long BatchId, Long sundryCreditorId, boolean b);


    @Query(
            value = "SELECT IFNULL(SUM(qty),0.0),IFNULL(SUM(return_qty),0.0) FROM `tranx_purchase_invoice_details_units_tbl` LEFT JOIN tranx_purchase_invoice_tbl " +
                    "ON tranx_purchase_invoice_details_units_tbl.purchase_invoice_id=tranx_purchase_invoice_tbl.id" +
                    " WHERE product_id=?1 AND batch_id=?2 AND rate=?3 AND tranx_purchase_invoice_details_units_tbl.status=?4 AND " +
                    "tranx_purchase_invoice_tbl.sundry_creditors_id=?5", nativeQuery = true
    )
    String findQuantity(Long productId, Long batchId, Double rate, boolean b, Long sundryCreditorId);

    @Query(
            value = "SELECT * FROM tranx_purchase_invoice_details_units_tbl LEFT JOIN tranx_purchase_invoice_tbl ON " +
                    "tranx_purchase_invoice_details_units_tbl.purchase_invoice_id=tranx_purchase_invoice_tbl.id WHERE " +
                    "product_id=?1 AND batch_id=?2 AND tranx_purchase_invoice_tbl.sundry_creditors_id=?3", nativeQuery = true
    )
    List<TranxPurInvoiceDetailsUnits> findProductTotalQty(Long productId, Long productBatchNoId, Long scId);
    ...
    TranxPurInvoiceDetailsUnits findByPurchaseTransactionIdAndProductIdAndStatus(Long id, Long productId, boolean b);
