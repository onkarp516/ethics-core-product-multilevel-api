...
public interface TranxSalesInvoiceDutiesTaxesRepository extends JpaRepository
    ...
    @Query(
            value = "SELECT * FROM tranx_sales_duties_taxes_tbl WHERE sales_invoice_id=?1 AND status=?2 ", nativeQuery = true
    )
    List<TranxSalesInvoiceDutiesTaxes> findTaxes(Long id, boolean b);
