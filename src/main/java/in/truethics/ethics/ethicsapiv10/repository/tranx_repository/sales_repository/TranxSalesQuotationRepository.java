...
public interface TranxSalesQuotationRepository extends JpaRepository<TranxSalesQuotation, Long> {
    ...
    @Query(
            value = "SELECT * FROM `tranx_sales_quotation_tbl` WHERE outlet_id=?1 AND DATE(bill_date) BETWEEN ?2 AND ?3 AND status=?4 ORDER by bill_date DESC", nativeQuery = true)
    List<TranxSalesQuotation> findSaleQuotationListWithDate(Long id, LocalDate startDatep, LocalDate endDatep, boolean b);

    @Query(
            value = "SELECT * FROM `tranx_sales_quotation_tbl` WHERE outlet_id=?1 AND branch_id=?2 AND DATE(bill_date) BETWEEN ?3 AND ?4 AND status=?5 ORDER by bill_date DESC", nativeQuery = true)
