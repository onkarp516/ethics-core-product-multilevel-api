...
//    List<LedgerMaster> findByOutletIdAndPrincipleGroupsId(Long id, Long i);
    ...
    List<LedgerMaster> findByUniqueCodeAndBranchIdAndOutletIdAndStatusAndColumnR(String baac, Long id, Long id1,
                                                                                 boolean status,boolean b);

    List<LedgerMaster> findByUniqueCodeAndBranchIdIsNullAndOutletIdAndStatusAndColumnR(String baac, Long id,
                                                                                       boolean status, boolean b);
    ...
    @Query(
            value = "SELECT sum(opening_bal) FROM `ledger_master_tbl` WHERE unique_code=?1 AND opening_bal_type=?2 AND outlet_id=?3 AND status=?4", nativeQuery = true
    )
    Double findSumOfOpeningUniqueCodeAndStatus(String caih,String type, Long id, boolean b);
    @Query(
            value = "SELECT * FROM `ledger_master_tbl` WHERE (principle_id=7 OR principle_id=8 OR principle_id=9) AND status=?1", nativeQuery = true
    )
    List<LedgerMaster> findRevenueLedgers(Boolean b);
